...
export async function systemRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/', async () => ({ name: 'SportsOS API', version: '0.3.2-refactor', endpoints: ['/', '/health', '/version', '/organizations', '/teams', '/media/logo', '/dashboard/stats'] }));
  app.get('/version', async () => ({ name: 'SportsOS API', version: '0.3.2-refactor', node: process.version }));
