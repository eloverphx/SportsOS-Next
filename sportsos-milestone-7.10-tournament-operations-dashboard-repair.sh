...
do
  grep -Fq "$marker" "$WORKSPACE" || {
    echo "ERROR: required Milestone 7.x UI marker missing from workspace: $marker" >&2
    exit 1
  }
done
...
[[ -f "$LIVE_CONTROL" ]] || {
  echo "ERROR: Milestone 7.7 live transition control missing: $LIVE_CONTROL" >&2
  exit 1
}

[[ -f "$FINAL_CONTROL" ]] || {
  echo "ERROR: Milestone 7.9 finalization control missing: $FINAL_CONTROL" >&2
  exit 1
}

grep -Fq 'live-game-transition-panel' "$LIVE_CONTROL" || {
  echo "ERROR: Milestone 7.7 live transition UI marker missing." >&2
  exit 1
}

grep -Fq 'game-result-finalization-panel' "$FINAL_CONTROL" || {
  echo "ERROR: Milestone 7.9 finalization UI marker missing." >&2
  exit 1
}
