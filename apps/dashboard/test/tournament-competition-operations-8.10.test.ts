...
describe("Milestone 8.10 tournament standings / bracket operations dashboard", () => {
  it("reports tournament completion when a champion resolves", () => {
    const summary = buildTournamentCompetitionOperationsSummary({
      totalTeams: 8,
      finalizedGames: 12,
      scheduledGames: 12,
      seededTeams: 8,
  ...
  it("renders the operations dashboard", () => {
    ...
    expect(component).toContain('data-testid="tournament-competition-operations-dashboard"');
  ...
  it("provides the tournament competition dashboard page", () => {
    const page = fs.readFileSync(
      new URL("../app/tournament/operations/page.tsx", import.meta.url),
      "utf8",
    );

    expect(page).toContain("Tournament Competition Dashboard");
    expect(page).toContain("TournamentCompetitionOperationsDashboard");
  });
});
