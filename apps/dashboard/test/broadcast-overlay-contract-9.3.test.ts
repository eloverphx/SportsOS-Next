...
describe("Milestone 9.3 broadcast overlay data contract", () => {
  it("normalizes authoritative game data into overlay version 1", () => {
    const snapshot = normalizeBroadcastOverlaySnapshot(
      {
        id: "game-93",
        status: "LIVE",
  ...
  it("rejects payloads without a game id", () => {
    ...
    ).toThrow("Broadcast overlay game payload is missing id.");
  });

  it("provides a per-game overlay API route", () => {
    const route = fs.readFileSync(
      new URL("../app/api/tournament/broadcast/overlay/[gameId]/route.ts", import.meta.url),
      "utf8",
    );

    expect(route).toContain("normalizeBroadcastOverlaySnapshot");
    expect(route).toContain("/games/${encodeURIComponent(gameId)}");
