...
describe("Milestone 9.5 realtime overlay updates", () => {
  it("uses Socket.IO for realtime updates", () => {
    ...
    expect(component).toContain('from "socket.io-client"');
  ...
  it("joins and leaves the selected game room", () => {
    ...
    expect(component).toContain('socket?.emit("game:join"');
    expect(component).toContain('socket.emit("game:leave"');
  });
  ...
  it("refreshes the authoritative snapshot when realtime game events arrive", () => {
    ...
    expect(component).toContain('socket.on("game:updated"');
    expect(component).toContain('socket.on("game:event"');
    expect(component).toContain('socket.on("scoreboard:update"');
  });
  ...
  it("keeps a fallback polling path", () => {
    ...
    expect(component).toContain("Fallback polling");
  });
  ...
  it("shows realtime connection state", () => {
    ...
    expect(component).toContain('data-testid="overlay-realtime-state"');
