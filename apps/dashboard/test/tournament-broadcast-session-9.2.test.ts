...
describe("Milestone 9.2 broadcast session API / operator UI", () => {
  it("provides a broadcast session API route", () => {
    const route = fs.readFileSync(
      new URL("../app/api/tournament/broadcast/session/route.ts", import.meta.url),
      "utf8",
    );
  ...
  it("renders the broadcast operator panel", () => {
    ...
    expect(component).toContain('data-testid="broadcast-operator-panel"');
  ...
  it("provides the tournament broadcast page", () => {
    const page = fs.readFileSync(
      new URL("../app/tournament/broadcast/page.tsx", import.meta.url),
      "utf8",
    );

    expect(page).toContain("Broadcast Operator");
    expect(page).toContain("TournamentBroadcastOperatorPanel");
  });
});
