...
describe("Milestone 9.8 broadcast operator theme controls", () => {
  it("normalizes operator theme settings", () => {
    const settings = normalizeBroadcastOverlayThemeSettings({
      homeAccent: "#123456",
      awayAccent: "#abcdef",
      showLogos: false,
      density: "COMPACT",
  ...
  it("provides a stable browser persistence key", () => {
    expect(BROADCAST_THEME_STORAGE_KEY).toBe("sportsos:broadcast-overlay-theme");
  });
  ...
  it("renders operator branding controls", () => {
    ...
    expect(component).toContain('data-testid="broadcast-theme-controls"');
  ...
  it("persists controls in localStorage", () => {
    ...
    expect(component).toContain("window.localStorage.getItem");
    expect(component).toContain("window.localStorage.setItem");
    expect(component).toContain("BROADCAST_THEME_STORAGE_KEY");
  });
});
