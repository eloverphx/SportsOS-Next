...
describe("Milestone 8.7 multi-round bracket construction", () => {
  it("builds all rounds for a four-team bracket", () => {
    ...
    const tree = buildTournamentBracketTree(seeded, []);
  ...
  it("propagates finalized winners into the championship", () => {
    ...
    expect(tree.rounds[1]?.matchups[0]?.homeSeed?.teamId).toBe("a");

    expect(tree.rounds[1]?.matchups[0]?.awaySeed?.teamId).toBe("c");
  });
  ...
  it("keeps unresolved next-round slots as TBD", () => {
    ...
    expect(tree.rounds[1]?.matchups[0]?.homeSeed).toBeNull();

    expect(tree.rounds[1]?.matchups[0]?.awaySeed).toBeNull();
  });

  it("propagates byes through the next round", () => {
    const seeded = seedBracket([row(1, "a", "A"), row(2, "b", "B"), row(3, "c", "C")]);
  ...
  it("resolves a champion once the final is complete", () => {
    const seeded = seedBracket([row(1, "a", "A"), row(2, "b", "B")]);
  ...
  it("returns an empty tree for an empty field", () => {
    const tree = buildTournamentBracketTree(seedBracket([]), []);
