...
describe("Milestone 7.8 delay / actual start tracking", () => {
  ...
  it("treats a start within 30 seconds as on time", () => {
    const timing = computeGameStartTiming(scheduled, "2026-08-16T20:00:20.000Z");
  ...
  it("calculates a late start", () => {
    const timing = computeGameStartTiming(scheduled, "2026-08-16T20:07:30.000Z");
  ...
  it("calculates an early start", () => {
    const timing = computeGameStartTiming(scheduled, "2026-08-16T19:57:00.000Z");
  ...
  it("rejects invalid timestamps", () => {
    expect(() => computeGameStartTiming("not-a-date", null)).toThrow(
      "Invalid scheduled start timestamp.",
    );

    expect(() => computeGameStartTiming(scheduled, "not-a-date")).toThrow(
      "Invalid actual start timestamp.",
    );
  });
});
