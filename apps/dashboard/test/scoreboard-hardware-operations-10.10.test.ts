...
import { buildScoreboardHardwareOperationsSummary } from "../lib/scoreboard-hardware-operations";

describe("Milestone 10.10 scoreboard hardware operations", () => {
  it("reports no-devices when nothing has connected", () => {
    const summary = buildScoreboardHardwareOperationsSummary([], []);

    expect(summary.stage).toBe("NO_DEVICES");
  ...
  it("renders assignment and reconcile controls", () => {
    ...
    expect(component).toContain('data-testid="scoreboard-hardware-operations"');
    expect(component).toContain("/scoreboard-devices/assignments");
    expect(component).toContain("/reconcile");
    expect(component).toContain("Reconcile Now");
  });

  it("provides the scoreboard hardware operations page", () => {
    const page = fs.readFileSync(
      new URL("../app/scoreboards/operations/page.tsx", import.meta.url),
      "utf8",
    );

    expect(page).toContain("Scoreboard Hardware Operations");
    expect(page).toContain("ScoreboardHardwareOperationsDashboard");
  });
});
