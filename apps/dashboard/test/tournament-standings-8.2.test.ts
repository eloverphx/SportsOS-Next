...
describe("Milestone 8.2 standings API / UI integration", () => {
  it("exposes a tournament standings API route", () => {
    const route = fs.readFileSync(
      new URL("../app/api/tournament/standings/route.ts", import.meta.url),
      "utf8",
    );
  ...
  it("keeps standings calculation in the shared 8.1 engine", () => {
    ...
    expect(route).toContain('from "../../../../lib/tournament-standings"');
  });

  it("renders the tournament standings table", () => {
    const component = fs.readFileSync(
      new URL("../components/tournament/TournamentStandingsTable.tsx", import.meta.url),
      "utf8",
    );

    expect(component).toContain('data-testid="tournament-standings-table"');
