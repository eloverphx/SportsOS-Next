...
describe("Milestone 9.4 browser source overlay", () => {
  it("provides a per-game browser-source route", () => {
    const page = fs.readFileSync(
      new URL("../app/broadcast/overlay/[gameId]/page.tsx", import.meta.url),
      "utf8",
    );
  ...
  it("polls the authoritative overlay API", () => {
    ...
    expect(component).toContain("/api/tournament/broadcast/overlay/");
  ...
  it("renders scoreboard-safe overlay elements", () => {
    ...
    expect(component).toContain('data-testid="broadcast-browser-overlay"');
    expect(component).toContain('data-testid="overlay-home-score"');
    expect(component).toContain('data-testid="overlay-away-score"');
    expect(component).toContain('data-testid="overlay-clock"');
  });
