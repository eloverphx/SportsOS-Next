...
test.describe("Milestone 7.1 tournament game operations workspace", () => {
  test("loads a scheduled game and exposes non-mutating operations context", async ({ page }) => {
    ...
    await page.route("**/api/tournament/game-operations/game-701", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
    ...
    await expect(page.getByTestId("game-operations-workspace")).toBeVisible();
    ...
    await expect(page.getByTestId("game-operations-readiness-count")).toHaveText("3/3");

    await expect(page.getByRole("button", { name: "Not enabled in 7.1" })).toHaveCount(4);
