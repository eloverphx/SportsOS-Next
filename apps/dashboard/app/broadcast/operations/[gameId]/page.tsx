...
export default function BroadcastFocusPage() {
  ...
  const gameId = useMemo(() => decodeURIComponent(params.gameId), [params.gameId]);
  ...
  const [retry, setRetry] = useState<CoordinatorRetry | null>(null);

  const [timeline, setTimeline] = useState<OperatorTimelineEvent[]>([]);
  ...
  const [recoveryOperator, setRecoveryOperator] = useState("");

  const [approveDestructiveRecovery, setApproveDestructiveRecovery] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    setSnapshot(snapshotJson?.data ?? null);

    setHealth(healthJson?.data?.health ?? null);

    setRetry(retryJson?.data?.retry ?? null);

    setTimeline(timelineJson?.data?.events ?? []);
  ...
  const runCoordinatorAction = useCallback(
    async (action: "prepare" | "reconcile" | "retry/execute" | "start" | "stop") => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(json?.error ?? "Coordinator action failed.");
        }
  ...
  const runGoLiveAction = useCallback(
    async (
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(json?.error ?? "Go-live action failed.");
        }
        ...
        setMessage(error instanceof Error ? error.message : "Go-live action failed.");
      } finally {
        setBusy(false);
      }
    },
  ...
  const executeRecovery = useCallback(async () => {
    if (!recoveryOperator.trim()) {
      setMessage("Operator name is required for controlled recovery.");
      return;
    }
    ...
    try {
      const response = await authenticatedRequest(
        `/broadcast-coordinator/${encodeURIComponent(gameId)}/recovery/execute`,
        {
          ...
          body: JSON.stringify({
            ...
            approveDestructive: approveDestructiveRecovery,
          }),
        },
      );
      ...
      if (!response.ok) {
        throw new Error(json?.error ?? "Controlled recovery failed.");
      }

      setMessage(json?.data?.message ?? "Controlled recovery completed.");

      setApproveDestructiveRecovery(false);

      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Controlled recovery failed.");
    } finally {
      setBusy(false);
    }
  }, [approveDestructiveRecovery, gameId, load, recoveryOperator]);
  ...
  const loadHandoffSummary = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json?.error ?? "Unable to load handoff summary.");
      }

      setHandoffSummary(json?.data ?? null);

      setMessage(null);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to load handoff summary.");
    } finally {
      setBusy(false);
    }
  }, [gameId]);

  const saveOperatorNote = useCallback(async () => {
    if (!handoffOperator.trim() || !handoffNote.trim()) {
      setMessage("Operator name and note are required.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json?.error ?? "Unable to save operator note.");
      }

      setHandoffNote("");

      setMessage("Operator note saved.");

      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to save operator note.");
    } finally {
      setBusy(false);
    }
  }, [gameId, handoffNote, handoffOperator, load]);
