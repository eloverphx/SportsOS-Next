...
export default function SecurityTelemetryPage() {
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch(`${API_BASE}/broadcast-coordinator/security-telemetry`, {
        cache: "no-store",
      });
      ...
      if (!response.ok) {
        throw new Error(json?.error ?? "Unable to load security telemetry.");
      }

      setData(json?.data ?? null);
      setError(null);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to load security telemetry.");
    }
  }, []);
  ...
  const sections = data
    ? ([
        ["Credential Rotation", data.sections.credentialRotation],
        ["Secret / Environment", data.sections.secretEnvironment],
        ["Secret Source", data.sections.secretSource],
        ["Session Invalidation", data.sections.sessionInvalidation],
      ] as const)
    : [];
  ...
  return (
    <main className="mx-auto max-w-7xl p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <a href="/broadcast/deployment" className="text-xs text-slate-500">
            ← Deployment Preflight
          </a>

          <h1 className="mt-2 text-2xl font-bold">Security Telemetry</h1>
      ...
      <section className="mt-6 rounded-xl border border-slate-800 p-5">
        <div className="text-xs text-slate-500">Security Gate</div>

        <div className="mt-1 text-2xl font-bold">{data?.ready ? "READY" : "BLOCKED"}</div>

        <div className="mt-3 text-xs text-slate-500">
          Required blockers: {data?.blockers?.length ? data.blockers.join(", ") : "none"}
        </div>
      </section>
            ...
            <div className="mt-4 space-y-2">
              {section.checks.map((check) => (
                <div key={check.id} className="rounded border border-slate-800 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <span className="text-xs font-semibold">{check.id}</span>

                    <span className="text-xs">{check.ok ? "PASS" : "FAIL"}</span>
                  </div>
