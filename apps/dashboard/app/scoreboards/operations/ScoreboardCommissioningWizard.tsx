...
type CommissioningRecord = {
  ...
  createdAt: string;
  updatedAt: string;
  status: "IN_PROGRESS" | "BLOCKED" | "GAME_READY";
  steps: CommissioningStep[];
};

const API_BASE = process.env.NEXT_PUBLIC_API_URL ?? "http://192.168.5.3:4001";

const COMMISSIONING_AUTO_REFRESH_MS = 5000;

const STEP_LABELS: Record<CommissioningStepId, string> = {
  FLASHED: "Firmware Flashed",
  PROVISIONED: "Device Provisioned",
  ENROLLED: "SportsOS Enrollment",
  VERIFIED: "Verified Hardware",
  ASSIGNED: "Scoreboard Assignment",
...
export function ScoreboardCommissioningWizard() {
  ...
  const completedCount = useMemo(
    () => commissioning?.steps.filter((step) => step.complete).length ?? 0,
    [commissioning],
  );

  const loadRecord = useCallback(async (requestedDeviceId: string) => {
    ...
    if (!response.ok) {
      throw new Error(`Commissioning load failed (${response.status}).`);
    }

    const json = await response.json();

    const record = json?.data?.commissioning ?? null;

    setCommissioning(record);

    return record as CommissioningRecord | null;
  }, []);
  ...
  async function startCommissioning() {
    ...
    if (!normalized) {
      setError("Enter a scoreboard device ID.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json?.error ?? `Commissioning start failed (${response.status}).`);
      }

      setCommissioning(json?.data?.commissioning ?? null);

      setError(null);
    } catch (startError) {
      setError(startError instanceof Error ? startError.message : "Unable to start commissioning.");
    } finally {
      setBusy(false);
    }
  }

  async function setManualStep(step: "FLASHED" | "PROVISIONED", complete: boolean) {
    if (!commissioning) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE}/scoreboard-device-commissioning/${encodeURIComponent(commissioning.deviceId)}/step`,
        {
          ...
          body: JSON.stringify({
            step,
            complete,
            note: complete ? "Confirmed by installer." : "Installer confirmation cleared.",
          }),
        },
      );
      ...
      if (!response.ok) {
        throw new Error(json?.error ?? `Step update failed (${response.status}).`);
      }
      ...
      setError(
        stepError instanceof Error ? stepError.message : "Unable to update commissioning step.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const validateCommissioningSilently = useCallback(async (targetDeviceId: string) => {
  ...
  async function runValidation() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json?.error ?? `Validation failed (${response.status}).`);
      }
  ...
  async function refresh() {
    ...
    try {
      await loadRecord(commissioning.deviceId);
  ...
  useEffect(() => {
    if (!commissioning || commissioning.status === "GAME_READY" || !autoRefreshEnabled) {
      return;
    }

    const device = commissioning.deviceId;

    const timer = window.setInterval(() => {
      void validateCommissioningSilently(device);
    }, COMMISSIONING_AUTO_REFRESH_MS);

    return () => {
      window.clearInterval(timer);
    };
  }, [commissioning, autoRefreshEnabled, validateCommissioningSilently]);

  return (
    <section className="mt-8 rounded-xl border border-slate-800 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">Scoreboard Commissioning</h2>
      ...
      {!commissioning ? (
        <div className="mt-5 flex flex-col gap-3 sm:flex-row">
          ...
          <button
            type="button"
            disabled={busy}
            onClick={() => void startCommissioning()}
        ...
        <>
          <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-800 p-4">
            <div>
              ...
              <div className="mt-1 font-mono text-sm">{commissioning.deviceId}</div>
            </div>

            <div className="text-sm text-slate-400">
              {completedCount}/{commissioning.steps.length} steps complete
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                Live Progress: {autoRefreshEnabled ? "ON" : "OFF"}
              </button>
          ...
          <div className="mt-4 space-y-2">
            {commissioning.steps.map((step) => (
              <div key={step.id} className="rounded-lg border border-slate-800 p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <div className="font-medium">{STEP_LABELS[step.id]}</div>
                ...
                {step.completedAt && (
                  <p className="mt-1 text-xs text-slate-500">Completed {step.completedAt}</p>
                )}

                {(step.id === "FLASHED" || step.id === "PROVISIONED") && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    <button
                      type="button"
                      disabled={busy || step.complete}
                      onClick={() => void setManualStep(step.id as "FLASHED" | "PROVISIONED", true)}
                    ...
                    <button
                      ...
                      disabled={busy || !step.complete}
                      onClick={() =>
                        void setManualStep(step.id as "FLASHED" | "PROVISIONED", false)
                      }
          ...
          {commissioning.status === "GAME_READY" && (
            <div className="mt-5 rounded-xl border border-slate-700 p-5">
              <div className="text-lg font-semibold">GAME_READY</div>
              <p className="mt-1 text-sm text-slate-400">
