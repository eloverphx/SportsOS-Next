...
function testAudioUrl(url: string): void {
...
export function BroadcastSessionPanel() {
  ...
  const [profile, setProfile] = useState<BroadcastSessionProfile | null>(null);
  const [title, setTitle] = useState("");
  const [sponsorUrl, setSponsorUrl] = useState("");
  const [enabled, setEnabled] = useState(true);
  const [showPowerPlay, setShowPowerPlay] = useState(true);
  const [showTeamLogos, setShowTeamLogos] = useState(true);
  const [scenePreset, setScenePreset] = useState<"STANDARD" | "MINIMAL" | "SPONSOR_FOCUS">(
    "STANDARD",
  );
  ...
  const loadProfile = useCallback(async (targetGameId: string) => {
    ...
    setSponsorUrlsText((nextProfile?.sponsorUrls ?? []).join("\n"));
    setSponsorRotationSeconds(nextProfile?.sponsorRotationSeconds ?? 10);
    setSoundEnabled(nextProfile?.soundEnabled ?? false);
    setGoalSoundUrl(nextProfile?.goalSoundUrl ?? "");
    setPenaltySoundUrl(nextProfile?.penaltySoundUrl ?? "");
    setHornSoundUrl(nextProfile?.hornSoundUrl ?? "");
    setIntermissionSoundUrl(nextProfile?.intermissionSoundUrl ?? "");
  }, []);
  ...
  async function saveProfile() {
    ...
    if (!normalized) {
      setError("Enter a game ID before saving broadcast settings.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json?.error ?? `Broadcast session save failed (${response.status}).`);
      }
      ...
      setError(
        saveError instanceof Error ? saveError.message : "Unable to save broadcast session.",
      );
    } finally {
      setBusy(false);
    }
  ...
  async function resetProfile() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Broadcast session reset failed (${response.status}).`);
      }
      ...
      setError(
        resetError instanceof Error ? resetError.message : "Unable to reset broadcast session.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const audioReadiness = useMemo(() => {
    const configured = [goalSoundUrl, penaltySoundUrl, hornSoundUrl, intermissionSoundUrl].filter(
    ...
    if (!soundEnabled) {
      return {
        ...
        detail: "Broadcast audio is intentionally muted.",
      };
    }
    ...
    if (configured === 0) {
      return {
        ...
        detail: "Audio is enabled but no sound URLs are configured.",
      };
    }
    ...
    return {
      ready: true,
      label: "Audio ready",
      detail: `${configured} sound source${configured === 1 ? "" : "s"} configured.`,
    };
  }, [soundEnabled, goalSoundUrl, penaltySoundUrl, hornSoundUrl, intermissionSoundUrl]);
  ...
  return (
    <section className="mt-8 rounded-xl border border-slate-800 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">Broadcast Session</h2>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <label className="text-sm">
          <span className="text-xs text-slate-500">Broadcast title</span>
          <input
            value={title}
        ...
        <label className="text-sm">
          <span className="text-xs text-slate-500">Sponsor image URL</span>
          <input
            value={sponsorUrl}
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-3">
        <label className="text-sm">
          <span className="text-xs text-slate-500">Scene preset</span>
          <select
            value={scenePreset}
            onChange={(event) =>
              setScenePreset(event.target.value as "STANDARD" | "MINIMAL" | "SPONSOR_FOCUS")
            }
            className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"
          >
        ...
        <label className="text-sm md:col-span-2">
          <span className="text-xs text-slate-500">Sponsor rotation URLs</span>
          <textarea
            value={sponsorUrlsText}
            onChange={(event) => setSponsorUrlsText(event.target.value)}
            rows={3}
            placeholder={"https://...\nhttps://..."}
            className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"
          />
        ...
        <label className="text-sm">
          <span className="text-xs text-slate-500">Rotation seconds</span>
          <input
            type="number"
            min={3}
            value={sponsorRotationSeconds}
            onChange={(event) => setSponsorRotationSeconds(Number(event.target.value) || 10)}
            className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"
          />
        </label>
      </div>
      ...
      <div className="mt-5 rounded-xl border border-slate-800 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="font-semibold">Broadcast Audio</div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <label className="text-sm">
            <span className="text-xs text-slate-500">Goal sound URL</span>
            <input
              value={goalSoundUrl}
              onChange={(event) => setGoalSoundUrl(event.target.value)}
          ...
          <label className="text-sm">
            <span className="text-xs text-slate-500">Penalty sound URL</span>
            <input
              value={penaltySoundUrl}
              onChange={(event) => setPenaltySoundUrl(event.target.value)}
          ...
          <label className="text-sm">
            <span className="text-xs text-slate-500">Horn sound URL</span>
            <input
              value={hornSoundUrl}
              onChange={(event) => setHornSoundUrl(event.target.value)}
          ...
          <label className="text-sm">
            <span className="text-xs text-slate-500">Intermission sound URL</span>
            <input
              value={intermissionSoundUrl}
              onChange={(event) => setIntermissionSoundUrl(event.target.value)}
      ...
      <div className="mt-4 rounded-lg border border-slate-800 p-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-sm font-semibold">Audio Readiness</div>
            <div className="mt-1 text-xs text-slate-500">{audioReadiness.detail}</div>
          </div>

          <span
            className={`rounded border px-3 py-1 text-xs font-medium ${
              audioReadiness.ready ? "border-slate-700" : "border-red-900/60 text-red-300"
            }`}
          >
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <button
            ...
            onClick={() => testAudioUrl(goalSoundUrl)}
          ...
          <button
            ...
            onClick={() => testAudioUrl(penaltySoundUrl)}
            className="rounded border border-slate-800 px-3 py-2 text-xs disabled:opacity-40"
          >
            Test Penalty Audio
          </button>
          ...
          <button
            ...
            onClick={() => testAudioUrl(hornSoundUrl)}
            className="rounded border border-slate-800 px-3 py-2 text-xs disabled:opacity-40"
          >
            Test Horn Audio
          </button>
          ...
          <button
            ...
            onClick={() => testAudioUrl(intermissionSoundUrl)}
            className="rounded border border-slate-800 px-3 py-2 text-xs disabled:opacity-40"
          >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        <label className="flex items-center gap-2 rounded-lg border border-slate-800 p-3 text-sm">
          <input
            ...
            onChange={(event) => setShowPowerPlay(event.target.checked)}
          />
          Show power play
        </label>
        ...
        <label className="flex items-center gap-2 rounded-lg border border-slate-800 p-3 text-sm">
          <input
            ...
            onChange={(event) => setShowTeamLogos(event.target.checked)}
          />
          Show team logos
        </label>
      </div>
