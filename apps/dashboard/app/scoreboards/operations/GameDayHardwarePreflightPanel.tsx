...
type PreflightCheck = {
  id: "COMMISSIONING" | "HEARTBEAT" | "RELIABILITY" | "SELF_TEST";
  passed: boolean;
  detail: string;
};
...
export function GameDayHardwarePreflightPanel() {
  ...
  const [latest, setLatest] = useState<GameDayHardwarePreflight | null>(null);

  const [history, setHistory] = useState<GameDayHardwarePreflight[]>([]);

  const [freshness, setFreshness] = useState<PreflightFreshness | null>(null);

  const [busy, setBusy] = useState(false);

  const [autoRerunEnabled, setAutoRerunEnabled] = useState(true);

  const autoRerunInFlight = useRef(false);

  const [countdownNow, setCountdownNow] = useState(() => Date.now());

  const [error, setError] = useState<string | null>(null);

  const loadHistory = useCallback(async (targetGameId: string) => {
    ...
    const [latestResponse, historyResponse] = await Promise.all([
      fetch(`${API_BASE}/game-day-hardware-preflight/${encodeURIComponent(normalized)}/latest`, {
        cache: "no-store",
      }),
      fetch(`${API_BASE}/game-day-hardware-preflight/${encodeURIComponent(normalized)}/history`, {
        cache: "no-store",
      }),
    ]);
    ...
    if (latestResponse.ok) {
      ...
      setLatest(json?.data?.preflight ?? null);

      setFreshness(json?.data?.freshness ?? null);
    }

    if (historyResponse.ok) {
      const json = await historyResponse.json();

      setHistory(json?.data?.preflights ?? []);
    }
  }, []);

  const runPreflightSilently = useCallback(
    async (targetGameId: string) => {
      if (autoRerunInFlight.current) {
        return;
      }

      autoRerunInFlight.current = true;
      ...
      try {
        ...
        if (json?.data?.preflight) {
          setLatest(json.data.preflight);
        }

        await loadHistory(targetGameId);
      } finally {
        autoRerunInFlight.current = false;
      }
    },
    [loadHistory],
  );
  ...
  async function runPreflight() {
    ...
    if (!normalized) {
      setError("Enter a game ID before running preflight.");
      return;
    }
    ...
    try {
      ...
      const result = json?.data?.preflight ?? null;
      ...
      if (!response.ok) {
        setError(json?.error ?? `Preflight failed (${response.status}).`);
      } else {
        setError(null);
      }
      ...
      setError(
        runError instanceof Error ? runError.message : "Unable to run game-day hardware preflight.",
      );
    } finally {
      setBusy(false);
    }
  ...
  useEffect(() => {
    ...
    if (!autoRerunEnabled || !normalizedGameId || !freshness?.fresh || !freshness.expiresAt) {
      return;
    }

    const remainingMs = Date.parse(freshness.expiresAt) - Date.now();
    ...
    const timer = window.setTimeout(() => {
      void runPreflightSilently(normalizedGameId);
    }, 1000);

    return () => {
      window.clearTimeout(timer);
    };
  }, [gameId, freshness, autoRerunEnabled, runPreflightSilently, countdownNow]);
  ...
  return (
    <section className="mt-8 rounded-xl border border-slate-800 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">Game-Day Hardware Preflight</h2>
      ...
      <div className="mt-5 rounded-xl border border-slate-800 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="font-semibold">Preflight Freshness</div>
          ...
          <span className="rounded border border-slate-700 px-2 py-1 text-xs font-semibold">
            {freshness?.fresh ? "FRESH" : "EXPIRED / REQUIRED"}
          </span>
        </div>
        ...
        {freshness?.ageMs != null && (
          <div className="mt-1 text-xs text-slate-500">
            Age: {Math.round(freshness.ageMs / 1000)}s
          </div>
        )}
      </div>
      ...
      <div className="mt-5 rounded-xl border border-slate-800 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="font-semibold">Start Window Guidance</div>
          ...
          <button
            type="button"
            onClick={() => setAutoRerunEnabled((current) => !current)}
            className="rounded border border-slate-700 px-3 py-1 text-xs font-medium"
          >
            Auto-Rerun: {autoRerunEnabled ? "ON" : "OFF"}
          </button>
            ...
            {freshness?.expiresAt
              ? (() => {
                  const remainingSeconds = Math.ceil(
                    Math.max(0, Date.parse(freshness.expiresAt) - countdownNow) / 1000,
                  );
        ...
        <p className="mt-3 text-sm text-slate-400">
          {!freshness
            ...
            : !freshness.fresh
              ...
              : freshness.expiresAt &&
                  Math.max(0, Date.parse(freshness.expiresAt) - countdownNow) <= 120000
                ? "Preflight is close to expiration. Rerun now to avoid a start delay."
                : freshness.expiresAt &&
                    Math.max(0, Date.parse(freshness.expiresAt) - countdownNow) <= 300000
                  ? "Preflight is still valid, but the start window is getting short."
                  : "Preflight is fresh and within the normal game-start window."}
        </p>

        {freshness?.fresh && freshness.expiresAt && (
          <div className="mt-3 text-xs text-slate-500">
            Current passing preflight expires at {freshness.expiresAt}.
          </div>
        )}
      </div>
      {latest && (
        <div className="mt-5 rounded-xl border border-slate-800 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="font-semibold">Latest Preflight</div>
              <div className="mt-1 text-xs text-slate-500">
                Device <span className="font-mono">{latest.deviceId}</span>
              </div>
            </div>

            <div className="text-xs text-slate-500">{latest.completedAt}</div>
          </div>
      ...
      {history.length > 0 && (
        <div className="mt-5 rounded-xl border border-slate-800 p-4">
          <div className="font-semibold">Preflight History</div>
          ...
          <div className="mt-3 overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {history.map((item) => (
                  <tr key={item.preflightId} className="border-t border-slate-800">
                    <td className="py-3 pr-4 text-xs text-slate-400">{item.completedAt}</td>
                    <td className="py-3 pr-4 font-mono text-xs">{item.deviceId}</td>
