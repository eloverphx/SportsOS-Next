...
export async function registerGoLiveSessionRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/go-live-sessions/:gameId/audit", async (request, reply) => {
    const gameId = (
      request.params as {
        gameId?: string;
      }
    ...
    return {
      success: true,
      data: {
        events: listGoLiveAuditEvents(gameId, Number.isFinite(limit) ? limit : 100),
      },
    };
  });

  app.get("/go-live-sessions/:gameId", async (request, reply) => {
    ...
    return {
      success: true,
      data: {
        session: getGoLiveSession(gameId),
      },
    };
  });

  app.put("/go-live-sessions/:gameId/health-hold", async (request, reply) => {
    const gameId = (
      request.params as {
        gameId?: string;
      }
    ...
    return {
      success: true,
      data: {
        session: configureGoLiveHealthHold(gameId, Number(body.seconds ?? 10)),
      },
    };
  });

  app.post("/go-live-sessions/:gameId/incident/acknowledge", async (request, reply) => {
    const gameId = (
      request.params as {
        gameId?: string;
      }
    ...
    if (current.status !== "DEGRADED") {
      return reply.code(409).send({
        success: false,
        error: "Only a DEGRADED go-live session can be acknowledged.",
      });
    }
    ...
    return {
      success: true,
      data: {
        session: acknowledgeGoLiveIncident(gameId, body.operator ?? null),
      },
    };
  });

  app.post("/go-live-sessions/:gameId/incident/retry-watchdog", async (request, reply) => {
    const gameId = (
      request.params as {
        gameId?: string;
      }
    ...
    const current = clearGoLiveIncidentAcknowledgement(gameId);
  ...
  app.post("/go-live-sessions/:gameId/watchdog", async (request, reply) => {
    const gameId = (
      request.params as {
        gameId?: string;
      }
    ...
    if (current.status === "LIVE" || current.status === "DEGRADED") {
      const encoderLive = runtime.session.status === "LIVE";

      const publishHealthy = runtime.telemetry.health === "HEALTHY";

      if (!encoderLive || !publishHealthy) {
        const reason = !encoderLive
          ? `Encoder session is ${runtime.session.status}.`
          : `Publish health is ${runtime.telemetry.health}.`;

        session = markGoLiveDegraded(gameId, reason);
        ...
        session = clearGoLiveDegraded(gameId);
    ...
    return {
      ...
      data: {
        ...
        runtime,
        watchdog: {
          healthy: session.status !== "DEGRADED",
  ...
  app.get("/go-live-sessions/:gameId/health-hold", async (request, reply) => {
    const gameId = (
      request.params as {
        gameId?: string;
      }
    ...
    const hold = evaluateGoLiveHealthHold({
      gameId,
      encoderLive: runtime.session.status === "LIVE",
      publishHealthy: runtime.telemetry.health === "HEALTHY",
    });
  ...
  app.put("/go-live-sessions/:gameId/auto-arm", async (request, reply) => {
    ...
    const session = configureGoLiveAutoArm({
      gameId,
  ...
  app.get("/go-live-sessions/:gameId/countdown", async (request, reply) => {
    const gameId = (request.params as { gameId?: string }).gameId?.trim();
    if (!gameId) return reply.code(400).send({ success: false, error: "Game ID is required." });
    return {
      success: true,
      data: { session: getGoLiveSession(gameId), countdown: evaluateGoLiveCountdown(gameId) },
    };
  });

  app.post("/go-live-sessions/:gameId/auto-arm/evaluate", async (request, reply) => {
    const gameId = (request.params as { gameId?: string }).gameId?.trim();
    if (!gameId) return reply.code(400).send({ success: false, error: "Game ID is required." });
    ...
    if (countdown.autoArmDue && ["IDLE", "COMPLETE", "ERROR"].includes(current.status)) {
      ...
      return reply.code(409).send({
        ...
        data: { autoArmed: false, session: current, countdown, preflight },
      });
    }
    return { success: true, data: { autoArmed: false, session: current, countdown } };
  });

  app.put("/go-live-sessions/:gameId/schedule", async (request, reply) => {
    const params = request.params as {
      gameId?: string;
    };
    ...
    try {
      const session = configureGoLiveSchedule({
        gameId,
        scheduledStartAt: body.scheduledStartAt ?? null,
        startWindowEarlyMinutes: body.startWindowEarlyMinutes,
        startWindowLateMinutes: body.startWindowLateMinutes,
      });
      ...
      return {
        ...
        data: {
          session,
          startWindow: evaluateGoLiveStartWindow(gameId),
        },
      };
    } catch {
      return reply.code(400).send({
        success: false,
        error: "Invalid scheduled start timestamp.",
      });
    }
  });

  app.get("/go-live-sessions/:gameId/start-window", async (request, reply) => {
    const params = request.params as {
      gameId?: string;
    };
  ...
  app.post("/go-live-sessions/:gameId/arm", async (request, reply) => {
    const params = request.params as {
      gameId?: string;
    };
    ...
    const gameDayPreflight = evaluateGameDayGoLivePreflight(gameId);

    if (!gameDayPreflight.ready) {
      return reply.code(409).send({
        success: false,
        error: "Game-day go-live preflight failed.",
        data: {
          preflight: gameDayPreflight,
        },
      });
    }
    ...
    if (!preflight.ready) {
      return reply.code(409).send({
        success: false,
        error: "Streaming readiness preflight must pass before arming go-live.",
        data: {
          preflight,
        },
      });
  ...
  app.post("/go-live-sessions/:gameId/start", async (request, reply) => {
    const params = request.params as {
      gameId?: string;
    };
    ...
    const startWindow = evaluateGoLiveStartWindow(gameId);

    if (!startWindow.withinWindow) {
      return reply.code(409).send({
        success: false,
        error: startWindow.tooEarly
    ...
    if (current.status === "EMERGENCY_STOPPED") {
      return reply.code(409).send({
        success: false,
        error: "Emergency-stopped go-live session must be reset before start.",
      });
    }
    ...
    if (current.status !== "ARMED") {
      return reply.code(409).send({
        success: false,
        error: "Go-live session must be ARMED before start.",
      });
    }
    ...
    if (!preflight.ready) {
      return reply.code(409).send({
        success: false,
        error: "Streaming readiness preflight failed before go-live start.",
        data: {
          preflight,
        },
      });
    ...
    if (!destination) {
      return reply.code(409).send({
        success: false,
        error: "Stream destination is missing.",
      });
    }
  ...
  app.post("/go-live-sessions/:gameId/confirm-live", async (request, reply) => {
    const params = request.params as {
      gameId?: string;
    };
    ...
    const healthHold = evaluateGoLiveHealthHold({
      gameId,
      encoderLive: runtime.session.status === "LIVE",
      publishHealthy: runtime.telemetry.health === "HEALTHY",
    });

    if (!healthHold.readyToConfirm) {
      return reply.code(409).send({
        success: false,
        error: "Publish health has not remained healthy for the required confirmation hold.",
        data: {
          runtime,
          healthHold,
        },
    ...
    if (runtime.session.status !== "LIVE" || runtime.telemetry.health !== "HEALTHY") {
      return reply.code(409).send({
        success: false,
        error: "Encoder must be LIVE with HEALTHY publish telemetry before go-live confirmation.",
        data: {
          runtime,
        },
      });
  ...
  app.post("/go-live-sessions/:gameId/emergency-stop", async (request, reply) => {
    const gameId = (
      request.params as {
        gameId?: string;
      }
  ...
  app.post("/go-live-sessions/:gameId/stop", async (request, reply) => {
    const params = request.params as {
      gameId?: string;
    };
  ...
  app.post("/go-live-sessions/:gameId/reset", async (request, reply) => {
    const params = request.params as {
      gameId?: string;
    };
