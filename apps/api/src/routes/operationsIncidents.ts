...
export const OPERATIONS_INCIDENTS_PATH = "/deployment/operations/incidents";

function isEnabled(): boolean {
  return process.env.SPORTSOS_OPERATIONS_STATUS_API_ENABLED?.trim().toLowerCase() === "true";
}
  ...
  if (!expected) {
    await reply.code(503).send({
      ...
      error: {
        ...
        message: "Protected operations status access is not configured.",
      },
    });
    return false;
  }
...
function setProtectedResponseHeaders(reply: FastifyReply): void {
  reply.header("Cache-Control", "no-store");
}

export async function registerOperationsIncidentRoutes(app: FastifyInstance): Promise<void> {
  app.get(OPERATIONS_INCIDENTS_PATH, async (request, reply) => {
      ...
      data: {
        ...
        summary: {
          ...
          warning: incidents.filter(
            (incident) => incident.severity === "warning" && incident.status !== "resolved",
          ).length,
        },
      },
    });
  ...
  app.get<{
  ...
  }>(`${OPERATIONS_INCIDENTS_PATH}/:incidentId`, async (request, reply) => {
    setProtectedResponseHeaders(reply);
    ...
    const incident = await findOperationsIncidentById(request.params.incidentId);
  ...
  app.post<{
  ...
  }>(`${OPERATIONS_INCIDENTS_PATH}/signals/escalation-delivery-failure`, async (request, reply) => {
    setProtectedResponseHeaders(reply);
    ...
    const incident = await openOrUpdateOperationsIncident({
      fingerprint: "operations:incident-escalation-delivery-failure",
      source: "operations",
      severity: "critical",
      title: "Incident escalation delivery failure",
      summary: "SportsOS could not deliver an incident escalation notification.",
  ...
  app.post<{
  ...
  }>(`${OPERATIONS_INCIDENTS_PATH}/:incidentId/acknowledge`, async (request, reply) => {
    setProtectedResponseHeaders(reply);
    if (!(await authorize(request, reply))) return;
    ...
    try {
      const incident = await acknowledgeOperationsIncident(request.params.incidentId, {
        actor,
  ...
  app.post<{
    Params: { incidentId: string };
    Body: { actor?: string; note?: string | null };
  }>(`${OPERATIONS_INCIDENTS_PATH}/:incidentId/resolve`, async (request, reply) => {
    setProtectedResponseHeaders(reply);
    if (!(await authorize(request, reply))) return;
    ...
    const incident = await resolveOperationsIncident(request.params.incidentId, {
      actor,
      note: request.body?.note ?? null,
    });
    if (!incident) {
