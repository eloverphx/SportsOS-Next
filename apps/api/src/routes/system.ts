...
export async function systemRoutes(app: FastifyInstance): Promise<void> {
  app.get("/dashboard/stats", { preHandler: requireAuth }, async () => {
    ...
    const [teamRows] = await pool.query<RowDataPacket[]>(
      "SELECT COUNT(*) AS count FROM teams WHERE active=TRUE",
    );
  ...
  app.get("/audit/recent", { preHandler: requireAuth }, async () => {
    const [rows] = await pool.query<RowDataPacket[]>(
      "SELECT id, action, details, created_at FROM audit_log ORDER BY id DESC LIMIT 20",
    );
    return { events: rows };
  });

  app.get("/health", async () => {
    const [mysqlStatus, redisStatus, mqttStatus, minioStatus] = await Promise.all([
      checkMysql(),
      checkRedis(),
      checkMqtt(),
      checkMinio(),
