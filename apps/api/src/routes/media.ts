...
export async function mediaRoutes(app: FastifyInstance): Promise<void> {
  app.get("/media/:id", async (request, reply) => {
    ...
    const [rows] = await pool.execute<RowDataPacket[]>(
      "SELECT object_key, mime_type FROM media_assets WHERE id=? LIMIT 1",
      [id.data],
    );
  ...
  app.post("/media/logo", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "Invalid logo upload", details: parsed.error.flatten() });
    const d = parsed.data;
    ...
    const [result] = await pool.execute<mysql.ResultSetHeader>(
      "INSERT INTO media_assets (organization_id, bucket, object_key, original_name, mime_type, size_bytes) VALUES (?, ?, ?, ?, ?, ?)",
      [d.organizationId || null, config.storage.bucket, key, d.fileName, d.mimeType, buffer.length],
    );
    await audit(authUser(request).sub, "logo.uploaded", {
