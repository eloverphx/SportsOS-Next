...
const STORE_FILE = path.join(DATA_DIR, "scoreboard-emergency-control-lock.json");
...
function load(): EmergencyPhysicalControlLock {
  try {
    const parsed = JSON.parse(fs.readFileSync(STORE_FILE, "utf8")) as EmergencyPhysicalControlLock;
    ...
    return {
      ...
      actorRoles: Array.isArray(parsed.actorRoles)
        ? parsed.actorRoles.filter((role): role is string => typeof role === "string")
        : [],
...
export function getEmergencyPhysicalControlLock(): EmergencyPhysicalControlLock {
  return {
    ...state,
