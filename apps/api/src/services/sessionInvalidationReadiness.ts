...
export function evaluateSessionInvalidationReadiness(
  ...
  checks.push({
    ...
    ok: jwt.length > 0,
    required: true,
    message: jwt ? "JWT secret is configured." : "JWT secret is missing.",
  });

  checks.push({
    id: "jwt:minimum-length",
    ok: jwt.length >= 32,
    required: true,
    message:
  ...
  return {
    ...
    strategy: "jwt-secret-rotation",
    impact: {
      activeJwtTokensInvalidated: true,
      usersMustSignInAgain: true,
      serverSessionStoreRequired: false,
    },
    checks,
  };
}
