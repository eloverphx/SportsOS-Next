...
export function evaluateBroadcastReleaseReadiness(
  ...
  checks.push({
    ...
    ok: env.PORT === "4001",
    required: true,
    message: env.PORT === "4001" ? "API runtime port is 4001." : "API runtime PORT must be 4001.",
  });
  ...
  checks.push({
    ...
    ok: env.HOST === "0.0.0.0",
    required: true,
    message: env.HOST === "0.0.0.0" ? "API binds to 0.0.0.0." : "API HOST must be 0.0.0.0.",
  });

  checks.push({
    id: "runtime:data-dir",
    ok: env.SPORTSOS_DATA_DIR === "/app/data",
    required: true,
    message:
  ...
  checks.push({
    id: "runtime:dashboard-origin",
    ok: configured(env.DASHBOARD_ORIGIN),
    required: true,
    message: configured(env.DASHBOARD_ORIGIN)
      ? "Dashboard origin is configured."
      : "DASHBOARD_ORIGIN is missing.",
  });

  checks.push({
    id: "runtime:public-api-url",
    ok: configured(env.PUBLIC_API_URL),
    required: true,
    message: configured(env.PUBLIC_API_URL)
  ...
  for (const name of [
    ...
    checks.push({
      id: `runtime:${name}`,
      ok: configured(env[name]),
      required: true,
      message: configured(env[name]) ? `${name} is configured.` : `${name} is missing.`,
    });
  }
