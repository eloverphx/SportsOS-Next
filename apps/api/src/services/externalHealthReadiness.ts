...
export function evaluateExternalHealthReadiness(
  ...
  const api = normalizeUrl(env.PUBLIC_API_URL);

  const dashboard = normalizeUrl(env.DASHBOARD_ORIGIN);
  ...
  const dashboardUrl = dashboard?.toString() ?? null;
  ...
  const checks = [
    {
      id: "dashboard:https",
      ok: dashboard?.protocol === "https:",
      required: true,
      message: "Dashboard external URL must use HTTPS.",
    },
    {
      id: "api:https",
      ok: api?.protocol === "https:",
      required: true,
      message: "API external URL must use HTTPS.",
    },
    {
      id: "targets:resolved",
      ok: targets.every((target) => Boolean(target.url)),
      required: true,
      message: "External dashboard and API-health targets must resolve from configuration.",
    },
  ];
