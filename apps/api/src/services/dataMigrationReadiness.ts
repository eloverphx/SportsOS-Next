...
export type DataMigrationReadinessResult = {
  ready: boolean;
  checks: DataMigrationReadinessCheck[];
};

export function evaluateDataMigrationReadiness(
  input: DataMigrationReadinessInput,
): DataMigrationReadinessResult {
  const checks: DataMigrationReadinessCheck[] = [];

  checks.push({
    id: "mysql:reachable",
    ok: input.mysqlReachable,
    required: true,
    message: input.mysqlReachable ? "MySQL is reachable." : "MySQL is not reachable.",
  });

  const dataDir = input.dataDir?.trim() ?? "";

  checks.push({
    id: "data-dir:configured",
    ok: Boolean(dataDir),
    required: true,
    message: dataDir
    ...
    for (const file of input.files) {
      ...
      if (!fs.existsSync(fullPath)) {
        checks.push({
          ...
          ok: true,
          required: false,
          message: `${file} does not exist yet; a new store can be created.`,
        });
      ...
      checks.push({
        ...
        ok: valid,
        required: true,
        message: valid ? `${file} is readable JSON.` : `${file} is unreadable or invalid JSON.`,
      });
    }
  }
