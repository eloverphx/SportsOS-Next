...
export function evaluatePublicExposureReadiness(
  ...
  const intendedPublicPaths = ["/", "/api/", "/api/health", "/socket.io/"];
  ...
  const checks = [
    {
      ...
      required: true,
      message: "Public exposure audit requires production runtime.",
    },
    {
      id: "api:public-url-configured",
      ok: Boolean(env.PUBLIC_API_URL),
      required: true,
      message: "PUBLIC_API_URL must be configured.",
    },
    {
      id: "dashboard:origin-configured",
      ok: Boolean(env.DASHBOARD_ORIGIN),
      required: true,
      message: "DASHBOARD_ORIGIN must be configured.",
    },
  ];
