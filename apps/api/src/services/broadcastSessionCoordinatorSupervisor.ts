...
import { runBroadcastCoordinatorSupervisorTick } from "./broadcastSessionCoordinator.js";

export type BroadcastCoordinatorSupervisorRuntimeOptions = {
  gameIds: () => string[];
  intervalMs?: number;
  onError?: (error: unknown, gameId: string) => void;
};

export function startBroadcastCoordinatorSupervisor(
  options: BroadcastCoordinatorSupervisorRuntimeOptions,
): () => void {
  const intervalMs = Math.max(1000, Math.min(options.intervalMs ?? 5000, 60000));
  ...
  const runTick = async (): Promise<void> => {
    ...
    for (const gameId of gameIds) {
      try {
        await runBroadcastCoordinatorSupervisorTick(gameId);
      } catch (error) {
        recordBroadcastCoordinatorAudit({
          gameId,
          type: "SUPERVISOR_TICK_FAILED",
          detail: error instanceof Error ? error.message : "Unknown supervisor tick failure.",
        });

        options.onError?.(error, gameId);
      }
    }
  };
