...
export type ScoreboardControlIncidentStatus = "OPEN" | "ACKNOWLEDGED" | "RESOLVED";
...
const STORE_FILE = path.join(DATA_DIR, "scoreboard-control-incident-resolution.json");
...
function loadStore(): Store {
  try {
    ...
    if (parsed.version !== 1 || !Array.isArray(parsed.resolutions)) {
      throw new Error("Invalid scoreboard control incident resolution store.");
    }
...
export function getScoreboardControlIncidentResolution(
  ...
  return store.resolutions.find((item) => item.auditId === auditId) ?? null;
}
...
export function setScoreboardControlIncidentResolution(input: {
  ...
  store.resolutions = store.resolutions.filter((item) => item.auditId !== input.auditId);
...
export function listScoreboardControlIncidentResolutions(): ScoreboardControlIncidentResolution[] {
  return [...store.resolutions].sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}
