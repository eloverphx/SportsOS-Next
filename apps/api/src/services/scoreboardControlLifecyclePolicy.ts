...
function extractStatus(payload: unknown): string | null {
  ...
  const candidates: unknown[] = [
    ...
    typeof value.data === "object" && value.data !== null
      ? (value.data as Record<string, unknown>).status
      : null,
    typeof value.data === "object" && value.data !== null
      ? (value.data as Record<string, unknown>).lifecycleStatus
      : null,
  ];
...
function isActiveStatus(status: string): boolean {
  return ["LIVE", "IN_PROGRESS", "STARTED", "ACTIVE", "RUNNING"].includes(status);
}

function isTerminalStatus(status: string): boolean {
  return ["FINAL", "FINISHED", "COMPLETED", "CANCELLED", "CANCELED", "POSTPONED"].includes(status);
}
...
export async function evaluateGameLifecyclePhysicalControlPolicy(
  ...
  const urls = [`/games/${encoded}`, `/games/${encoded}/state`, `/games/${encoded}/snapshot`];
  ...
  for (const url of urls) {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      return {
        allowed: false,
        status: null,
        reason: "Unable to verify authoritative game lifecycle.",
      };
    }
    ...
    if (!status) {
      ...
      return {
        allowed: false,
        status: null,
        reason: "Authoritative game lifecycle status is unavailable.",
      };
    }
    ...
    if (isTerminalStatus(status)) {
      return {
        allowed: false,
        status,
        reason: `Physical controls are locked because game status is ${status}.`,
      };
    }
    ...
    return {
      allowed: false,
      status,
      reason: `Physical controls are locked until the game is active (status: ${status}).`,
    };
  }
