...
export function resolveTrustedProxyConfig(env: Record<string, string | undefined>): string[] {
  const raw = env.SPORTSOS_TRUST_PROXY?.trim();
...
export function evaluateTrustedProxyReadiness(
  ...
  const trustProxy = resolveTrustedProxyConfig(env);

  return {
    ready: trustProxy.length > 0,
    configured: Boolean(env.SPORTSOS_TRUST_PROXY?.trim()),
    trustProxy,
    forwardedHeaders: {
      proto: "x-forwarded-proto",
      host: "x-forwarded-host",
    ...
    message: env.SPORTSOS_TRUST_PROXY?.trim()
