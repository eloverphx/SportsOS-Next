...
import type { ScoreboardPhysicalControlPolicy } from "@sportsos/core";

export type ScoreboardControlPolicyAuditAction = "SET" | "DELETE";

export type ScoreboardControlPolicyAuditRecord = {
  auditId: string;
  action: ScoreboardControlPolicyAuditAction;
  actorUserId: string | null;
  actorRoles: string[];
  previousPolicy: ScoreboardPhysicalControlPolicy | null;
  nextPolicy: ScoreboardPhysicalControlPolicy | null;
  reason: string | null;
  createdAt: string;
};
...
type Store = {
  version: 1;
  records: ScoreboardControlPolicyAuditRecord[];
};

const DATA_DIR = process.env.SPORTSOS_DATA_DIR ?? path.resolve(process.cwd(), "data");

const STORE_FILE = path.join(DATA_DIR, "scoreboard-control-policy-audit.json");
...
function loadStore(): Store {
  try {
    ...
    if (parsed.version !== 1 || !Array.isArray(parsed.records)) {
      throw new Error("Invalid scoreboard control policy audit store.");
    }
...
export function recordScoreboardControlPolicyAudit(
  record: ScoreboardControlPolicyAuditRecord,
