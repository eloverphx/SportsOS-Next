...
export function evaluateRollbackRestoreReadiness(
  ...
  const checks: RollbackRestoreCheck[] = [];

  const composeFile = path.join(input.root, "docker-compose.yml");

  const smokeScript = path.join(input.root, "scripts/release-smoke-test.sh");

  checks.push({
    id: "rollback:compose-present",
    ok: fs.existsSync(composeFile),
    required: true,
    message: "docker-compose.yml must be present for rollback.",
  });

  checks.push({
    id: "rollback:smoke-test-present",
    ok: fs.existsSync(smokeScript),
    required: true,
    message: "Release smoke-test script must be present.",
  });

  const dataDir = input.dataDir?.trim() ?? "";

  checks.push({
    id: "restore:data-dir-configured",
    ok: Boolean(dataDir),
    required: true,
    message: dataDir
  ...
  checks.push({
    id: "restore:backup-dir-configured",
    ok: Boolean(backupDir),
    required: true,
    message: backupDir
