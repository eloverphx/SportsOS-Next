...
export function evaluateGameStartPreflight(
  ...
  const freshness = gameDayHardwarePreflightFreshness(preflight);
  ...
  if (!preflight) {
    return {
      ...
      message: "Run and pass a game-day hardware preflight before starting the game.",
      preflightId: null,
      deviceId: null,
      expiresAt: null,
    };
  ...
  if (currentDeviceId && !matchesCurrentAssignment(preflight, gameId, currentDeviceId)) {
    return {
      allowed: false,
      gameId,
      code: "PREFLIGHT_ASSIGNMENT_CHANGED",
      message:
  ...
  if (!freshness.fresh || !freshness.expiresAt) {
    return {
      ...
      message: "The latest passing game-day hardware preflight has expired. Rerun preflight.",
      preflightId: preflight.preflightId,
      deviceId: preflight.deviceId,
      expiresAt: freshness.expiresAt,
    };
