...
const dispatches = new Map<string, CommissioningSelfTestDispatch>();

export function createCommissioningSelfTestDispatch(
  deviceId: string,
): CommissioningSelfTestDispatch {
  const commandId = `commissioning-self-test-${crypto.randomUUID()}`;

  const dispatch: CommissioningSelfTestDispatch = {
    commandId,
    deviceId,
    status: "PENDING",
  ...
  dispatches.set(commandId, dispatch);
...
export function getCommissioningSelfTestDispatch(
  ...
  const dispatch = dispatches.get(commandId);
...
export function acknowledgeCommissioningSelfTestDispatch(
  ...
  if (!dispatch) {
    throw new Error("Self-test command not found.");
  }

  if (dispatch.deviceId !== deviceId) {
    throw new Error("Self-test command belongs to another device.");
  }
...
export function completeCommissioningSelfTestDispatch(
  ...
  dispatch.status = passed ? "COMPLETED" : "FAILED";
  dispatch.completedAt = new Date().toISOString();
  dispatch.resultTestId = resultTestId;
