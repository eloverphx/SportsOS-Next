...
function sourceConfigured(): boolean {
  return Boolean(
    process.env.SPORTSOS_ENCODER_SOURCE_URL?.trim() ||
      process.env.SPORTSOS_ENCODER_SOURCE_URL_TEMPLATE?.trim(),
  );
}

export function evaluateStreamingReadiness(gameId: string): StreamingReadinessPreflight {
  const destination = getStreamDestinationProfile(gameId);

  const runtime = encoderRuntimeSnapshot(gameId);

  const checks: StreamingReadinessCheck[] = [
    {
      id: "DESTINATION_PRESENT",
    ...
    },
    {
      ...
      passed: Boolean(destination?.enabled),
      message: destination?.enabled ? "Streaming is enabled." : "Streaming is disabled.",
    },
    {
      id: "INGEST_URL",
      passed: Boolean(destination?.ingestUrl?.trim()),
      message: destination?.ingestUrl ? "Ingest URL is configured." : "Ingest URL is missing.",
    },
    {
      id: "CREDENTIAL_REFERENCE",
      passed: Boolean(destination?.credentialRef?.trim()),
      message: destination?.credentialRef
        ? "Credential reference is configured."
        : "Credential reference is missing.",
    },
    {
      id: "DESTINATION_PROBE",
      passed: destination?.status === "READY" || destination?.status === "LIVE",
      message:
        destination?.status === "READY" || destination?.status === "LIVE"
