import { encoderRuntimeSnapshot } from "./encoderRuntime.js";
...
import { evaluateStreamingReadiness } from "./streamingReadinessPreflight.js";
...
export function evaluateGameDayGoLivePreflight(gameId: string): GameDayGoLivePreflight {
  const session = getGoLiveSession(gameId);

  const streaming = evaluateStreamingReadiness(gameId);
  ...
  const checks: GameDayGoLiveCheck[] = [
    {
      id: "STREAMING_PREFLIGHT",
      passed: streaming.ready,
    ...
    {
      id: "START_WINDOW",
      passed: startWindow.withinWindow,
      message: startWindow.withinWindow
    ...
    {
      id: "DEGRADED_INCIDENT",
      passed: session.status !== "DEGRADED",
      message:
        session.status === "DEGRADED"
          ? (session.degradationReason ?? "Live incident remains degraded.")
          : "No degraded live incident is active.",
    },
    {
      id: "RECOVERY_EXHAUSTION",
      passed: runtime.recovery.state !== "EXHAUSTED",
      message:
        runtime.recovery.state === "EXHAUSTED"
          ? "Encoder automatic recovery is exhausted."
          : "Encoder recovery remains available.",
    },
    {
      id: "ENCODER_AVAILABILITY",
      passed: runtime.session.status === "STOPPED" || runtime.session.status === "ERROR",
      message:
    ...
    {
      id: "SCHEDULE_COUNTDOWN",
      passed:
        !countdown.scheduled ||
        countdown.secondsUntilStart == null ||
        countdown.secondsUntilStart >= -Math.max(session.startWindowLateMinutes, 0) * 60,
      message: !countdown.scheduled
        ? "No scheduled start restricts this session."
        : `Scheduled start countdown is ${String(countdown.secondsUntilStart)} seconds.`,
    },
  ];
