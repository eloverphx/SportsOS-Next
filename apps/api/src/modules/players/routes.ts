...
export async function playerRoutes(app: FastifyInstance): Promise<void> {
  app.get("/players", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!query.success)
      return reply
        .code(400)
        .send({ error: "Invalid player filters", details: query.error.flatten() });
  ...
  app.post("/players", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "Invalid player data", details: parsed.error.flatten() });
    ...
    if (relationError === "team")
      return reply.code(400).send({ error: "Team does not belong to the selected organization" });
    const player = await createPlayer(parsed.data);
    await audit(authUser(request).sub, "player.created", {
      playerId: player.id,
  ...
  app.put("/players/:id", { preHandler: requireAuth }, async (request, reply) => {
    ...
    const parsed = playerSchema.safeParse(request.body);
    if (!id.success)
      return reply.code(400).send({ error: "Invalid player id", details: id.error.flatten() });
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "Invalid player data", details: parsed.error.flatten() });
