...
function mapAvailablePlayer(row: RowDataPacket): Player {
  ...
  return {
    ...
    shoots:
      row.shoots === null || row.shoots === undefined ? null : (row.shoots as Player["shoots"]),
    birthDate: null,
...
export async function listAvailablePlayers(filters: {
  ...
  if (search) {
    searchSql =
      " AND (p.first_name LIKE ? OR p.last_name LIKE ? OR p.preferred_name LIKE ? OR CAST(p.jersey_number AS CHAR) LIKE ?)";
    const like = `%${search}%`;
    params.push(like, like, like, like);
  }
  const [rows] = await pool.execute<RowDataPacket[]>(
    `SELECT p.*, o.name AS organization_name, t.name AS team_name
    FROM players p
    ...
    [filters.seasonId, filters.teamId, filters.organizationId, ...params.slice(3)],
  );
...
export async function deleteRosterEntry(id: number): Promise<boolean> {
  const [result] = await pool.execute<mysql.ResultSetHeader>(
    "DELETE FROM team_rosters WHERE id = ?",
    [id],
  );
  return result.affectedRows > 0;
}

export async function validateRosterRelationships(
  input: Pick<RosterInput, "seasonId" | "teamId" | "playerId">,
): Promise<"season" | "team" | "player" | "organization" | null> {
  const [rows] = await pool.execute<RowDataPacket[]>(
    `SELECT s.organization_id AS season_org, t.organization_id AS team_org, p.organization_id AS player_org
    FROM seasons s JOIN teams t ON t.id = ? JOIN players p ON p.id = ? WHERE s.id = ?`,
    [input.teamId, input.playerId, input.seasonId],
  );
  const row = rows[0];
  if (!row) {
    const [season] = await pool.execute<RowDataPacket[]>("SELECT id FROM seasons WHERE id = ?", [
      input.seasonId,
    ]);
    if (!season[0]) return "season";
    const [team] = await pool.execute<RowDataPacket[]>("SELECT id FROM teams WHERE id = ?", [
      input.teamId,
    ]);
  ...
  if (
    Number(row.season_org) !== Number(row.team_org) ||
    Number(row.team_org) !== Number(row.player_org)
  )
    return "organization";
  return null;
}

export async function rosterEntryExists(
  seasonId: number,
  teamId: number,
  playerId: number,
  excludeId?: number,
  ...
  const [rows] = await pool.execute<RowDataPacket[]>(
    `SELECT id FROM team_rosters WHERE season_id = ? AND team_id = ? AND player_id = ?${excludeSql}`,
    params,
  );
  return Boolean(rows[0]);
}

export async function jerseyConflict(
  seasonId: number,
  teamId: number,
  ...
  const [rows] = await pool.execute<RowDataPacket[]>(
    ...
    WHERE season_id = ? AND team_id = ? AND jersey_number = ? AND active = TRUE${excludeSql}`,
    params,
  );
  return Boolean(rows[0]);
}
