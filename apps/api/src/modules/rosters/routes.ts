...
export async function rosterRoutes(app: FastifyInstance): Promise<void> {
  app.get("/rosters", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "Invalid roster filters", details: parsed.error.flatten() });
  ...
  app.get("/rosters/available", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "Invalid player filters", details: parsed.error.flatten() });
  ...
  app.post("/rosters", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "Invalid roster data", details: parsed.error.flatten() });
        ...
        .send({
          error:
            relationshipError === "organization"
              ...
              : `${relationshipError[0]?.toUpperCase()}${relationshipError.slice(1)} not found`,
        });
    if (await rosterEntryExists(parsed.data.seasonId, parsed.data.teamId, parsed.data.playerId))
      return reply.code(409).send({ error: "Player is already on this roster" });
    if (
      parsed.data.active &&
      ...
      return reply
        .code(409)
        .send({ error: "Jersey number is already assigned on this active roster" });
    const entry = await createRosterEntry(parsed.data);
    await audit(authUser(request).sub, "roster.created", {
      rosterId: entry.id,
      teamId: entry.teamId,
  ...
  app.put("/rosters/:id", { preHandler: requireAuth }, async (request, reply) => {
    ...
    await audit(authUser(request).sub, "roster.updated", {
      rosterId: entry.id,
      teamId: entry.teamId,
      seasonId: entry.seasonId,
    });
  ...
  app.delete("/rosters/:id", { preHandler: requireAuth }, async (request, reply) => {
    ...
    await audit(authUser(request).sub, "roster.deleted", {
      rosterId: id.data,
