...
export async function listSeasons(filters: {
  ...
  const [rows] = await pool.execute<RowDataPacket[]>(
    `${SELECT_SEASON}${where} ORDER BY s.start_date DESC, s.name`,
    params,
  );
...
export async function deleteSeason(id: number): Promise<boolean> {
  const [result] = await pool.execute<mysql.ResultSetHeader>("DELETE FROM seasons WHERE id = ?", [
    id,
  ]);
...
export async function organizationExists(organizationId: number): Promise<boolean> {
  const [rows] = await pool.execute<RowDataPacket[]>("SELECT id FROM organizations WHERE id = ?", [
    organizationId,
  ]);
