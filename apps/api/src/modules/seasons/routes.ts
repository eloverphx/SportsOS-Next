...
export async function seasonRoutes(app: FastifyInstance): Promise<void> {
  app.get("/seasons", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (query.active !== undefined && active === undefined)
      return reply.code(400).send({ error: "Invalid active filter" });
    return {
  ...
  app.post("/seasons", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "Invalid season data", details: parsed.error.flatten() });
    ...
    await audit(authUser(request).sub, "season.created", {
      seasonId: season.id,
      name: season.name,
    });
