...
describe("Tournament scheduling 6.4 contract", () => {
  it("limits interactive moves to scheduled games", () => {
    ...
    expect(editor).toContain("Only scheduled games can be moved from Tournament Director.");
  });
  ...
  it("blocks hard conflicts until explicitly overridden", () => {
    ...
    expect(compactEditor).toContain("want to override the block");
  ...
  it("fetches the authoritative full game before issuing PUT", () => {
    ...
    expect(editor).toContain("regulationPeriodLengthMs: game.regulationPeriodLengthMs");
  ...
  it("is mounted in Tournament Director and refreshes after save", () => {
    ...
    expect(page).toContain("<TournamentScheduleEditor games={games} onSaved={load} />");
  });
});
