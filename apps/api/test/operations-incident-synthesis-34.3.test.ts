...
import { listOperationsIncidents } from "../src/services/operationsIncidentJournal.js";

const tempRoots: string[] = [];

async function withTempJournal(): Promise<void> {
  const root = await mkdtemp(path.join(os.tmpdir(), "sportsos-incident-synth-"));
  tempRoots.push(root);
...
describe("Milestone 34.3 incident synthesis", () => {
  it("creates deterministic candidates from reliability and recovery telemetry", () => {
    const candidates = synthesizeOperationsIncidentCandidates({
  ...
  it("falls back to a warning incident when operations runs failed without stronger signals", () => {
    ...
    expect(candidates[0]?.fingerprint).toBe("operations:recent-run-failure");
  ...
  it("persists repeated synthesized signals into one deduplicated incident", async () => {
    ...
    const first = await persistSynthesizedOperationsIncidents(status, "2026-08-28T21:00:00.000Z");
    const second = await persistSynthesizedOperationsIncidents(status, "2026-08-28T21:01:00.000Z");
    ...
    expect(incidents[0]?.events.map((event) => event.type)).toEqual(["opened", "updated"]);
  });
