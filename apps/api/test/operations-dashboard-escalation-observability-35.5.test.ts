...
describe("Milestone 35.5 dashboard escalation observability", () => {
  it("types incident escalation status data", () => {
    ...
    expect(helper).toContain("SPORTSOS_M35_5_ESCALATION_STATUS_TYPES");
    expect(helper).toContain("IncidentEscalationStatus");
    expect(helper).toContain("SPORTSOS_M35_5_5_SNAPSHOT_ESCALATION_TYPE");
    expect(helper).toContain("incidentEscalation?: IncidentEscalationStatus");
  });
  ...
  it("renders escalation delivery telemetry without a JSX wrapper", () => {
    ...
    expect(page).toContain("SPORTSOS_M35_5_ESCALATION_DASHBOARD");
    ...
    expect(page).toContain("SPORTSOS_M35_5_4_RESPONSE_DATA_BINDING");
    expect(page).toContain("response.data?.incidentEscalation");
    expect(page).not.toContain("response.incidentEscalation");
    expect(page).not.toContain("SPORTSOS_M35_5_1_ESCALATION_STATUS_BINDING");
  });

  it("does not render webhook configuration or bearer tokens", () => {
    const page = read("apps/dashboard/app/dashboard/operations/page.tsx");

    expect(page).not.toContain("SPORTSOS_INCIDENT_ESCALATION_WEBHOOK_URL");
