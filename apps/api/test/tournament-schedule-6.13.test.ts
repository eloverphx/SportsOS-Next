...
import { PERMISSIONS, ROLES, roleHasPermission } from "../src/modules/auth/index.js";
...
const compactEditor = editor.replace(/\s+/g, " ");

describe("Tournament scheduling 6.13 override authorization", () => {
  it("defines schedule override as a distinct permission", () => {
    expect(PERMISSIONS.GAME_SCHEDULE_OVERRIDE).toBe("game.schedule.override");
    expect(PERMISSIONS.GAME_SCHEDULE_OVERRIDE).not.toBe(PERMISSIONS.GAME_MANAGE);
  });

  it("grants override authority only to system admins and organization owners", () => {
    expect(roleHasPermission(ROLES.SYSTEM_ADMIN, PERMISSIONS.GAME_SCHEDULE_OVERRIDE)).toBe(true);

    expect(roleHasPermission(ROLES.ORGANIZATION_OWNER, PERMISSIONS.GAME_SCHEDULE_OVERRIDE)).toBe(
      true,
    );

    expect(roleHasPermission(ROLES.ORGANIZATION_ADMIN, PERMISSIONS.GAME_SCHEDULE_OVERRIDE)).toBe(
      false,
    );

    expect(roleHasPermission(ROLES.TEAM_ADMIN, PERMISSIONS.GAME_SCHEDULE_OVERRIDE)).toBe(false);

    expect(roleHasPermission(ROLES.SCOREKEEPER, PERMISSIONS.GAME_SCHEDULE_OVERRIDE)).toBe(false);
  });
  ...
  it("requires elevated permission on both create and update override requests", () => {
    ...
    for (const route of [post, put]) {
      ...
      expect(route).toContain("permission: PERMISSIONS.GAME_SCHEDULE_OVERRIDE");
    }
  });
  ...
  it("keeps normal scheduling separate from override authority in the UI", () => {
    expect(editor).toContain("userHasPermission(user, PERMISSIONS.GAME_MANAGE)");
    expect(editor).toContain("PERMISSIONS.GAME_SCHEDULE_OVERRIDE");
    expect(editor).toContain("canOverrideHardConflicts");
    expect(compactEditor).toContain("An organization owner or system administrator must approve");
  });
});
