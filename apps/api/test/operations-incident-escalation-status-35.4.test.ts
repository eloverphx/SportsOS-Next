...
describe("Milestone 35.4 escalation delivery telemetry", () => {
  it("ships a dedicated escalation telemetry collector", () => {
    const collector = read("scripts/operations-incident-escalation-status.sh");

    expect(collector).toContain("SPORTSOS_M35_4_ESCALATION_STATUS_COLLECTOR");
  ...
  it("merges telemetry into the existing operations snapshot", () => {
    ...
    expect(status).toContain("SPORTSOS_M35_4_ESCALATION_STATUS_MERGE");
    expect(status).toContain("SPORTSOS_M35_4_1_STATUS_OWNERSHIP");
