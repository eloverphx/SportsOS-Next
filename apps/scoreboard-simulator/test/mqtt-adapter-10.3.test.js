...
import { applySimulatorCommand, createInitialSimulatorState } from "../src/mqtt-adapter.js";

test("10.3 applies SET_SCORE commands", () => {
  const state = createInitialSimulatorState("scoreboard-simulator-1");

  const next = applySimulatorCommand(state, {
...
test("10.3 applies clock and period commands", () => {
  let state = createInitialSimulatorState("scoreboard-simulator-1");

  state = applySimulatorCommand(state, {
    deviceId: "scoreboard-simulator-1",
    sentAt: new Date().toISOString(),
    command: {
      protocolVersion: 1,
...
test("10.3 applies full SYNC_STATE", () => {
  ...
  assert.equal(next.clock.remainingMs, 45000);
});
