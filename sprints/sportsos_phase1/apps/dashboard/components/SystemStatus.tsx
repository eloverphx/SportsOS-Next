...
export function SystemStatus() {
  ...
  useEffect(() => {
    const load = () => fetch(`${apiUrl}/health`).then(r => r.json()).then(setHealth).catch(() => setHealth(null));
  ...
  const services: Record<string, State> = health?.services ?? { mysql:'pending', redis:'pending', mqtt:'pending', minio:'pending' };
  services.realtime = realtime;
  return <div className="grid">{Object.entries(services).map(([name,status]) =>
    <div className="card" key={name}><span className={`dot ${status}`} /><strong>{name.toUpperCase()}</strong><p>{status}</p></div>
  )}</div>;
}
