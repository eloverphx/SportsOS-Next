...
const teamSchema = z.object({
  organizationId: z.number().int().positive(), name: z.string().trim().min(2).max(160),
  nickname: z.string().trim().max(100).nullable().optional(), sport: z.string().trim().min(2).max(80).default('Hockey'),
  division: z.string().trim().max(100).nullable().optional(), season: z.string().trim().max(40).nullable().optional(),
  homeArena: z.string().trim().max(160).nullable().optional(), primaryColor: color.default('#ef4444'),
  secondaryColor: color.default('#0f172a'), logoAssetId: z.number().int().positive().nullable().optional(), active: z.boolean().default(true)
});
...
function mapTeam(row: RowDataPacket) {
  return {
    ...
    logoUrl: logoUrl(row.logo_asset_id ? Number(row.logo_asset_id) : null), active: Boolean(row.active),
...
export async function teamRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/teams/:id', { preHandler: requireAuth }, async (request, reply) => {
    ...
    const [rows] = await pool.execute<RowDataPacket[]>('SELECT t.*, o.name AS organization_name FROM teams t JOIN organizations o ON o.id=t.organization_id WHERE t.id=?', [id.data]);
  ...
  app.post('/teams', { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Invalid team data', details: parsed.error.flatten() });
    const d = parsed.data;
    const [result] = await pool.execute<mysql.ResultSetHeader>('INSERT INTO teams (organization_id, name, nickname, sport, division, season, home_arena, primary_color, secondary_color, logo_asset_id, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [d.organizationId, d.name, d.nickname || null, d.sport, d.division || null, d.season || null, d.homeArena || null, d.primaryColor, d.secondaryColor, d.logoAssetId || null, d.active]);
  ...
  app.put('/teams/:id', { preHandler: requireAuth }, async (request, reply) => {
    ...
    const d = parsed.data;
    const [result] = await pool.execute<mysql.ResultSetHeader>('UPDATE teams SET organization_id=?, name=?, nickname=?, sport=?, division=?, season=?, home_arena=?, primary_color=?, secondary_color=?, logo_asset_id=?, active=? WHERE id=?', [d.organizationId, d.name, d.nickname || null, d.sport, d.division || null, d.season || null, d.homeArena || null, d.primaryColor, d.secondaryColor, d.logoAssetId || null, d.active, id.data]);
  ...
  app.delete('/teams/:id', { preHandler: requireAuth }, async (request, reply) => {
    const id = z.coerce.number().int().positive().safeParse((request.params as { id: string }).id);
    if (!id.success) return reply.code(400).send({ error: 'Invalid team id' });
    const [result] = await pool.execute<mysql.ResultSetHeader>('DELETE FROM teams WHERE id=?', [id.data]);
    if (!result.affectedRows) return reply.code(404).send({ error: 'Team not found' });
