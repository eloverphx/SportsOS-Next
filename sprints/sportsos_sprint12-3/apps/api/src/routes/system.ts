...
export async function systemRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/', async () => ({ name: 'SportsOS API', version: '0.3.3-player-engine', endpoints: ['/', '/health', '/version', '/organizations', '/teams', '/players', '/media/logo', '/dashboard/stats'] }));
  app.get('/version', async () => ({ name: 'SportsOS API', version: '0.3.3-player-engine', node: process.version }));
