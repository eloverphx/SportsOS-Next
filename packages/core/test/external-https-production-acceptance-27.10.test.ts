...
describe("Milestone 27.10 external HTTPS production acceptance", () => {
  ...
  const proxyContract = fs.readFileSync(
    new URL("../../../apps/api/src/services/reverseProxyRouteContract.ts", import.meta.url),
    "utf8",
  );

  const tls = fs.readFileSync(
    new URL("../../../apps/api/src/services/tlsCertificateReadiness.ts", import.meta.url),
    "utf8",
  );

  const realtime = fs.readFileSync(
    new URL("../../../apps/api/src/services/externalRealtimeReadiness.ts", import.meta.url),
    "utf8",
  );

  const exposure = fs.readFileSync(
    new URL("../../../apps/api/src/services/publicExposureReadiness.ts", import.meta.url),
    "utf8",
  );

  it("retains strict external-live rehearsal mode", () => {
    expect(rehearsal).toContain("SPORTSOS_REQUIRE_EXTERNAL_LIVE");
    expect(rehearsal).toContain("scripts/tls-certificate-check.sh");
    expect(rehearsal).toContain("scripts/external-health-check.sh");
    expect(rehearsal).toContain("scripts/external-realtime-check.sh");
    expect(rehearsal).toContain("scripts/public-exposure-audit.sh");
  });
