...
describe("Milestone 25.10 deployment / release readiness acceptance", () => {
  const readiness = fs.readFileSync(
    new URL("../../../apps/api/src/services/broadcastReleaseReadiness.ts", import.meta.url),
    "utf8",
  );

  const secrets = fs.readFileSync(
    new URL("../../../apps/api/src/services/secretEnvironmentValidation.ts", import.meta.url),
    "utf8",
  );

  const smoke = fs.readFileSync(
    new URL("../../../scripts/release-smoke-test.sh", import.meta.url),
    "utf8",
  );
  ...
  const artifact = fs.readFileSync(
    new URL("../../../scripts/generate-release-artifact.sh", import.meta.url),
    "utf8",
  );

  const rehearsal = fs.readFileSync(
    new URL("../../../scripts/staging-production-rehearsal.sh", import.meta.url),
    "utf8",
  );

  const dashboard = fs.readFileSync(
    new URL("../../../apps/dashboard/app/broadcast/deployment/page.tsx", import.meta.url),
    "utf8",
  );
