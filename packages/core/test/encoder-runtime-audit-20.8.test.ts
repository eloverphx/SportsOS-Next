...
describe("Milestone 20.8 encoder runtime audit / failure history", () => {
  ...
  it("persists bounded encoder audit history", () => {
    expect(audit).toContain("encoder-runtime-audit.json");
  ...
  it("records operator start and stop requests", () => {
    expect(route).toContain('"START_REQUESTED"');

    expect(route).toContain('"STOP_REQUESTED"');
  });

  it("provides an audit endpoint", () => {
    expect(route).toContain('"/encoder-sessions/:gameId/audit"');

    expect(route).toContain("listEncoderAuditEvents");
  });

  it("shows encoder history in the operator UI", () => {
    expect(panel).toContain("Encoder Runtime History");

    expect(panel).toContain("/audit?limit=12");
  });
});
