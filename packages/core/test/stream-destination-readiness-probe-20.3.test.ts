...
describe("Milestone 20.3 stream destination readiness / connection probe", () => {
  const probe = fs.readFileSync(
    new URL("../../../apps/api/src/services/streamDestinationProbe.ts", import.meta.url),
    "utf8",
  );
  ...
  it("persists probe readiness", () => {
    expect(profile).toContain("lastProbeAt");

    expect(profile).toContain("lastProbeLatencyMs");
  });

  it("provides an operator probe route", () => {
    expect(route).toContain('"/stream-destinations/:gameId/probe"');
  });

  it("provides dashboard probe controls", () => {
    expect(panel).toContain("Probe Destination");

    expect(panel).toContain("Last probe");
  });

  it("does not transmit credentials from the probe service", () => {
    expect(probe).not.toContain("credentialRef");
  });
});
