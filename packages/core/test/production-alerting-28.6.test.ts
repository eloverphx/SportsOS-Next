...
describe("Milestone 28.6 production alerting", () => {
  const script = fs.readFileSync(
    new URL("../../../scripts/production-alert-check.sh", import.meta.url),
    "utf8",
  );
  ...
  it("extracts failures into an incident record", () => {
    expect(script).toContain("grep '^FAIL  '");

    expect(script).toContain("latest-alert.txt");
  });

  it("deduplicates alerts with cooldown", () => {
    expect(script).toContain("SPORTSOS_ALERT_COOLDOWN_SECONDS");

    expect(script).toContain("sha256sum");

    expect(script).toContain("SUPPRESSED");
  });

  it("supports optional webhook delivery", () => {
    expect(script).toContain("SPORTSOS_ALERT_WEBHOOK_URL");
