...
describe("Milestone 13.9 device-offer handler replacement repair", () => {
  ...
  const routeStart = source.indexOf('"/scoreboard-firmware/device-offer"');
  ...
  it("declares rollout before rollout.releaseId", () => {
    ...
    const firstUse = route.indexOf("rollout.releaseId");
  ...
  it("requires verified devices", () => {
    expect(route).toContain("isVerifiedDevice");

    expect(route).toContain("Verified scoreboard device required.");
  });

  it("returns no offer without an active rollout", () => {
    expect(route).toContain("findActiveRolloutForDevice");

    expect(route).toContain("rollout: null");

    expect(route).toContain("updateAvailable: false");
  });

  it("selects the rollout release directly", () => {
    expect(route).toContain("getFirmwareRelease");

    expect(route).toContain("rollout.releaseId");
  });

  it("returns device-bound artifact URL when update is available", () => {
    expect(route).toContain("artifactUrl");

    expect(route).toContain("deviceId=");

    expect(route).toContain("updateAvailable: true");
  });
});
