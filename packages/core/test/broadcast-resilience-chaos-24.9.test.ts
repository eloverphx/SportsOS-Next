...
import { evaluateBroadcastRuntimeHeartbeat } from "../../../apps/api/src/services/broadcastRuntimeHeartbeat";

import { evaluateBroadcastResilienceSupervisor } from "../../../apps/api/src/services/broadcastResilienceSupervisor";

import { classifyStreamDestinationFailure } from "../../../apps/api/src/services/streamDestinationFailurePolicy";

import { evaluateResilienceRetryBudget } from "../../../apps/api/src/services/broadcastResilienceRetryBudget";
...
describe("Milestone 24.9 failure injection / chaos regression tests", () => {
  it("fails safe when heartbeat disappears during intended live state", () => {
    const result = evaluateBroadcastResilienceSupervisor({
    ...
    expect(result.heartbeat.state).toBe("MISSING");

    expect(result.recovery.action).toBe("require-operator-review");

    expect(result.recovery.automatic).toBe(false);
  });
  ...
  it("fails safe when runtime heartbeat is stale", () => {
    ...
    expect(result.heartbeat.state).toBe("STALE");
  ...
  it("does not silently kill unexpected live runtime", () => {
    ...
    expect(result.action).toBe("request-controlled-stop");
  ...
  it("treats destination auth failure as non-retryable", () => {
    const failure = classifyStreamDestinationFailure({
      ok: false,
      statusCode: 401,
    });

    const budget = evaluateResilienceRetryBudget({
      attempts: 0,
      failure,
    });
    ...
    expect(budget.state).toBe("REFUSED");

    expect(budget.retryAllowed).toBe(false);
  });
  ...
  it("backs off transient remote failure", () => {
    ...
    expect(budget.state).toBe("SCHEDULED");

    expect(budget.delayMs).toBe(20_000);
  });
  ...
  it("treats malformed heartbeat timestamp conservatively", () => {
    const result = evaluateBroadcastRuntimeHeartbeat({
      runtimeStatus: "LIVE",
      lastActivityAt: "definitely-not-a-date",
    });
