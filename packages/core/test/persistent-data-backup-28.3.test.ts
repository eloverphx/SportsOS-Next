...
describe("Milestone 28.3 persistent data backups", () => {
  const script = fs.readFileSync(
    new URL("../../../scripts/backup-persistent-data.sh", import.meta.url),
    "utf8",
  );

  it("backs up MinIO host data mount", () => {
    expect(script).toContain('eq .Destination "/data"');

    expect(script).toContain("sportsos-minio-");
  });

  it("backs up SportsOS persistent data", () => {
    expect(script).toContain("sportsos-data-");

    expect(script).toContain("--exclude='data/backups'");
  });
  ...
  it("implements retention", () => {
    expect(script).toContain("SPORTSOS_DATA_BACKUP_RETENTION_DAYS");
  ...
  it("does not print MinIO password", () => {
    expect(script).not.toContain('echo "$MINIO_PASSWORD"');
  });
});
