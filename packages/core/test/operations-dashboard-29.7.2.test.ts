...
describe("Milestone 29.7.2 dashboard relative import repair", () => {
  ...
  it("uses a colocated relative server helper", () => {
    ...
    expect(page).not.toContain('from "@/lib/operationsStatus"');
    expect(helper).toContain('import "server-only"');
  });

  it("explicitly types issue rendering", () => {
    expect(page).toContain("issue: ReliabilityIssue");
    expect(page).toContain("index: number");
  });
  ...
  it("keeps protected API access server-side", () => {
    ...
    expect(helper).toContain("/deployment/operations/status");
