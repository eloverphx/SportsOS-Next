...
import { evaluateTlsCertificateReadiness } from "../../../apps/api/src/services/tlsCertificateReadiness";

describe("Milestone 27.5 TLS certificate readiness", () => {
  it("accepts valid production HTTPS targets", () => {
    const result = evaluateTlsCertificateReadiness({
    ...
    expect(result.minimumDaysRemaining).toBe(14);
  });
  ...
  it("deduplicates same TLS hostname", () => {
    ...
    expect(result.targets).toEqual(["sports.example.com"]);
  });
