...
describe("Milestone 15.5 policy change audit / actor attribution", () => {
  const service = fs.readFileSync(
    new URL("../../../apps/api/src/services/scoreboardControlPolicyAudit.ts", import.meta.url),
    "utf8",
  );
  ...
  it("persists dedicated policy audit records", () => {
    expect(service).toContain("scoreboard-control-policy-audit.json");

    expect(service).toContain("previousPolicy");

    expect(service).toContain("nextPolicy");
  });
