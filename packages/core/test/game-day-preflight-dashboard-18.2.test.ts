...
describe("Milestone 18.2 game-day preflight dashboard / operator workflow", () => {
  ...
  it("runs preflight by game ID", () => {
    expect(panel).toContain("Run Game-Day Preflight");

    expect(panel).toContain("/game-day-hardware-preflight/");
  });

  it("shows all preflight checks", () => {
    for (const check of ["COMMISSIONING", "HEARTBEAT", "RELIABILITY", "SELF_TEST"]) {
  ...
  it("surfaces pass/fail and failure detail", () => {
    ...
    expect(panel).toContain("check.detail");
  });

  it("loads latest preflight and history", () => {
    expect(panel).toContain("/latest");

    expect(panel).toContain("/history");

    expect(panel).toContain("Preflight History");
  });

  it("renders on scoreboard operations", () => {
    expect(page).toContain("GameDayHardwarePreflightPanel");
