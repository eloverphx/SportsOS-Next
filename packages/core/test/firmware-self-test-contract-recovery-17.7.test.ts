...
describe("Milestone 17.7 firmware self-test contract recovery", () => {
  const header = fs.readFileSync(
    new URL("../../../firmware/esp32-scoreboard/include/CommissioningSelfTest.h", import.meta.url),
    "utf8",
  );
  ...
  it("restores the firmware commissioning self-test contract", () => {
    expect(header).toContain("CommissioningSelfTestTelemetry");

    expect(header).toContain("CommissioningSelfTest");
  });
  ...
  it("serializes device telemetry as JSON", () => {
    expect(implementation).toContain('document["deviceId"]');

    expect(implementation).toContain("serializeJson");
  });

  it("keeps the self-test separate from game state", () => {
    expect(implementation).toContain("non-game-state-changing");
  });
});
