...
describe("Milestone 13.4 device OTA update check / offer binding", () => {
  it("adds a verified-device OTA offer endpoint", () => {
    ...
    expect(routes).toContain("/scoreboard-firmware/device-offer");
  ...
  it("returns a device-bound firmware artifact URL", () => {
    ...
    expect(routes).toContain("artifactUrl");

    expect(routes).toContain("deviceId=");
  });

  it("defines an ESP32 firmware update check client", () => {
    const header = fs.readFileSync(
      new URL("../../../firmware/esp32-scoreboard/include/FirmwareUpdateClient.h", import.meta.url),
      "utf8",
    );

    expect(header).toContain("FirmwareUpdateClient");

    expect(header).toContain("FirmwareUpdateCheckState");
  });
  ...
  it("validates offers before marking an update available", () => {
    ...
    expect(source).toContain("FirmwareUpdateContract::validateOffer");
  ...
  it("checks for updates only after enrollment verification", () => {
    ...
    expect(main).toContain("firmwareUpdateClient->loop");

    expect(main).toContain("enrollmentClient->isVerified()");
  });
  ...
  it("adds host simulator coverage for OTA offers", () => {
    ...
    expect(simulator).toContain("evaluateFirmwareUpdateOffer");

    expect(simulator).toContain("BLOCKED_UNVERIFIED");

    expect(simulator).toContain("UPDATE_AVAILABLE");
  });
});
