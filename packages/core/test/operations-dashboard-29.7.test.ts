...
describe("Milestone 29.7 authenticated operations dashboard", () => {
  const libPath = fs.existsSync("apps/dashboard/src/lib/operationsStatus.ts")
    ? "apps/dashboard/src/lib/operationsStatus.ts"
    : "apps/dashboard/lib/operationsStatus.ts";

  const pagePath = fs.existsSync("apps/dashboard/src/app/dashboard/operations/page.tsx")
  ...
  it("is disabled separately by default", () => {
    expect(lib).toContain("SPORTSOS_OPERATIONS_DASHBOARD_ENABLED");
    expect(lib).toContain('process.env.SPORTSOS_OPERATIONS_DASHBOARD_ENABLED === "true"');
  });

  it("uses the protected deployment endpoint", () => {
    expect(lib).toContain("/deployment/operations/status");
  });
