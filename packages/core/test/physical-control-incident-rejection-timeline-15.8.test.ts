...
describe("Milestone 15.8 physical control incident / rejection timeline", () => {
  ...
  it("derives incidents from rejected or errored control audit records", () => {
    expect(audit).toContain("listScoreboardControlIncidents");
    expect(audit).toContain('record.disposition === "REJECTED"');
    expect(audit).toContain("Boolean(record.error)");
  });

  it("exposes an authorized incident endpoint", () => {
    expect(route).toContain("/scoreboard-control-incidents");
  ...
  it("renders an operator incident timeline", () => {
    expect(panel).toContain("Physical Control Incident Timeline");
    expect(panel).toContain("No physical-control incidents recorded.");
    expect(panel).toContain("incident.deviceId");
    expect(panel).toContain("incident.error");
  });
