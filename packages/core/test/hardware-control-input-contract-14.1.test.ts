...
describe("Milestone 14.1 hardware control input contract", () => {
  it("defines protocol version 1", () => {
    expect(SCOREBOARD_CONTROL_INPUT_PROTOCOL_VERSION).toBe(1);
  });
  ...
  it("supports score clock period and horn intents", () => {
    for (const type of [
      ...
      expect(isScoreboardControlInputType(type)).toBe(true);
    }
  });

  it("defines server disposition contract", () => {
    const source = fs.readFileSync(
      new URL("../src/scoreboard-control-input-contract.ts", import.meta.url),
      "utf8",
    );
    ...
    expect(source).toContain('"IGNORED_DUPLICATE"');
  });
  ...
  it("defines firmware-side control input contract", () => {
    ...
    expect(header).toContain("ScoreboardControlInputEvent");

    expect(header).toContain("sequence");

    expect(header).toContain("inputId");
  });
  ...
  it("keeps physical control as intent rather than direct authority", () => {
    ...
    expect(readme).toContain("do **not** directly mutate authoritative SportsOS game state");
  });
});
