...
describe("Milestone 18.9 preflight auto-rerun / start-window refresh", () => {
  ...
  it("enables auto-rerun by default", () => {
    expect(panel).toContain("autoRerunEnabled");

    expect(panel).toContain("useState(true)");
  });

  it("reruns when two minutes or less remain", () => {
    expect(panel).toContain("remainingMs >");
  ...
  it("prevents overlapping background reruns", () => {
    expect(panel).toContain("autoRerunInFlight");
  });

  it("allows the operator to pause auto-rerun", () => {
    expect(panel).toContain("Auto-Rerun:");

    expect(panel).toContain("Auto-rerun is paused");
  });
