...
describe("Milestone 24.6 stream destination failure handling", () => {
  ...
  it("classifies auth failures as operator review", () => {
    const result = classifyStreamDestinationFailure({
      ok: false,
      statusCode: 401,
    });
  ...
  it("classifies transient network failures", () => {
    ...
    expect(result.failureClass).toBe("TRANSIENT_NETWORK");

    expect(result.action).toBe("RETRY_ALLOWED");
  });
  ...
  it("provides destination failure classification API", () => {
    ...
    expect(route).toContain('"/broadcast-coordinator/:gameId/destination-failure/classify"');

    expect(route).toContain("classifyStreamDestinationFailure");
  });
});
