...
describe("Milestone 24.7 resilience retry budgets / backoff policy", () => {
  ...
  it("schedules retry within budget", () => {
    ...
    const result = evaluateResilienceRetryBudget({
      attempts: 0,
  ...
  it("provides retry-budget API", () => {
    ...
    expect(route).toContain('"/broadcast-coordinator/:gameId/resilience-retry-budget"');

    expect(route).toContain("evaluateResilienceRetryBudget");
  });
});
