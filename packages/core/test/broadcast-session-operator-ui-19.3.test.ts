...
describe("Milestone 19.3 broadcast session operator UI", () => {
  ...
  it("loads and saves broadcast sessions", () => {
    expect(panel).toContain("/broadcast-sessions/");

    expect(panel).toContain("Save Broadcast Session");
  });

  it("provides branding and presentation controls", () => {
    expect(panel).toContain("Broadcast title");

    expect(panel).toContain("Sponsor image URL");

    expect(panel).toContain("Show power play");

    expect(panel).toContain("Show team logos");
  });

  it("provides reset behavior", () => {
    expect(panel).toContain("Reset to Overlay Defaults");
  ...
  it("renders on scoreboard operations", () => {
    expect(page).toContain("BroadcastSessionPanel");
