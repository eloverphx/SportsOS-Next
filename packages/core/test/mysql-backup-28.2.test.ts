...
describe("Milestone 28.2 automated MySQL backups", () => {
  const script = fs.readFileSync(
    new URL("../../../scripts/backup-mysql.sh", import.meta.url),
    "utf8",
  );
  ...
  it("implements retention", () => {
    expect(script).toContain("SPORTSOS_MYSQL_BACKUP_RETENTION_DAYS");
  ...
  it("does not print database password", () => {
    expect(script).not.toContain('echo "$MYSQL_PASSWORD"');
  });
});
