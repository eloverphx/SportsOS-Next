...
describe("Milestone 16.4 readiness recovery / restored-service events", () => {
  ...
  it("emits a restoration event only after a not-ready state", () => {
    expect(monitor).toContain('"DEVICE_READINESS_RESTORED"');
  ...
  it("projects both degradation and restoration readiness events", () => {
    expect(audit).toContain("listScoreboardControlReadinessEvents");

    expect(audit).toContain('"DEVICE_READINESS_DEGRADED"');

    expect(audit).toContain('"DEVICE_READINESS_RESTORED"');
  });

  it("exposes an authorized readiness event endpoint", () => {
    expect(route).toContain("/scoreboard-control-readiness-events");

    expect(route).toContain('"CONTROL_POLICY_READ"');
  });

  it("shows readiness recovery timeline in operator UI", () => {
    expect(panel).toContain("Readiness Recovery Timeline");

    expect(panel).toContain("RESTORED");
