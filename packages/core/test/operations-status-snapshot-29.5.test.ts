...
describe("Milestone 29.5 operations status snapshot", () => {
  const script = fs.readFileSync(
    new URL("../../../scripts/operations-status-snapshot.sh", import.meta.url),
    "utf8",
  );

  it("refreshes reliability before generating the snapshot", () => {
    expect(script).toContain("scripts/operations-reliability-scorecard.sh");
  });

  it("creates a stable latest snapshot", () => {
    expect(script).toContain('LATEST_FILE="${SNAPSHOT_DIR}/latest.json"');
