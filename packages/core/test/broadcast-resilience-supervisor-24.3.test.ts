...
describe("Milestone 24.3 coordinator/runtime reconciliation supervisor", () => {
  it("keeps healthy live state consistent", () => {
    ...
    expect(result.heartbeat.state).toBe("HEALTHY");

    expect(result.recovery.action).toBe("observe");
  });
  ...
  it("provides read-only supervisor API", () => {
    ...
    expect(route).toContain('"/broadcast-coordinator/:gameId/resilience-supervisor"');
