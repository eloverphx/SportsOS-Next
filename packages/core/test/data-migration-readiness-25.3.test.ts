...
import { evaluateDataMigrationReadiness } from "../../../apps/api/src/services/dataMigrationReadiness";
...
describe("Milestone 25.3 database / persistent data migration readiness", () => {
  it("passes with reachable mysql and writable empty data directory", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sportsos-25.3-"));

    const result = evaluateDataMigrationReadiness({
      dataDir: dir,
      mysqlReachable: true,
      files: ["optional-store.json"],
    });
  ...
  it("provides migration-readiness API", () => {
    ...
    expect(route).toContain('"/broadcast-coordinator/data-migration-readiness"');

    expect(route).toContain("SELECT 1");

    expect(route).toContain("evaluateDataMigrationReadiness");
  });
});
