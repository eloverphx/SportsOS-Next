...
describe("Milestone 16.5 readiness flap detection / stability window", () => {
  ...
  it("defines a configurable readiness stability window", () => {
    expect(monitor).toContain("SPORTSOS_READINESS_STABILITY_WINDOW_MS");

    expect(monitor).toContain('"20000"');
  });

  it("tracks pending transitions separately from committed readiness", () => {
    expect(monitor).toContain("pendingState");

    expect(monitor).toContain("firstObservedAtMs");
  });

  it("does not emit transition events until the observed state is stable", () => {
    expect(monitor).toContain("stableForMs");

    expect(monitor).toContain("requiredStableMs");
  ...
  it("exposes the configured stability window", () => {
    expect(route).toContain("/scoreboard-control-readiness-stability");
