...
import { evaluateExternalHealthReadiness } from "../../../apps/api/src/services/externalHealthReadiness";

describe("Milestone 27.6 external API/dashboard health readiness", () => {
  it("derives dashboard and API health targets", () => {
    const result = evaluateExternalHealthReadiness({
    ...
    expect(result.targets).toEqual([
      ...
      {
        id: "api-health",
        url: "https://sports.example.com/api/health",
        required: true,
      },
    ]);
  });
  ...
  it("supports separate API hostname", () => {
    ...
    expect(result.targets.find((target) => target.id === "api-health")?.url).toBe(
