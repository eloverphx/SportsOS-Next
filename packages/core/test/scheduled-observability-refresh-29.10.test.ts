...
describe("Milestone 29.10 scheduled observability refresh", () => {
  const refresh = fs.readFileSync("scripts/refresh-operations-observability.sh", "utf8");
  ...
  it("refreshes severity metrics before status snapshot", () => {
    const metrics = refresh.indexOf('bash "$METRICS"');
    const snapshot = refresh.indexOf('bash "$SNAPSHOT"');
  ...
  it("accepts severity exit codes as generated states", () => {
    ...
    expect(refresh).toContain("A warning/critical severity is data, not a refresh failure.");
  });

  it("still fails on unexpected metrics errors", () => {
    expect(refresh).toContain("severity metrics generation failed");
