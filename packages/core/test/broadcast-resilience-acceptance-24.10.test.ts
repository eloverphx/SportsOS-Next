...
describe("Milestone 24.10 production resilience acceptance", () => {
  const policy = fs.readFileSync(
    new URL("../../../apps/api/src/services/broadcastRecoveryPolicy.ts", import.meta.url),
    "utf8",
  );

  const heartbeat = fs.readFileSync(
    new URL("../../../apps/api/src/services/broadcastRuntimeHeartbeat.ts", import.meta.url),
    "utf8",
  );

  const supervisor = fs.readFileSync(
    new URL("../../../apps/api/src/services/broadcastResilienceSupervisor.ts", import.meta.url),
    "utf8",
  );

  const controlled = fs.readFileSync(
    new URL("../../../apps/api/src/services/broadcastControlledRecovery.ts", import.meta.url),
    "utf8",
  );

  const snapshot = fs.readFileSync(
    new URL("../../../apps/api/src/services/broadcastRecoverySnapshotStore.ts", import.meta.url),
    "utf8",
  );

  const destination = fs.readFileSync(
    new URL("../../../apps/api/src/services/streamDestinationFailurePolicy.ts", import.meta.url),
    "utf8",
  );

  const budget = fs.readFileSync(
    new URL("../../../apps/api/src/services/broadcastResilienceRetryBudget.ts", import.meta.url),
    "utf8",
  );
  ...
  const focus = fs.readFileSync(
    new URL("../../../apps/dashboard/app/broadcast/operations/[gameId]/page.tsx", import.meta.url),
    "utf8",
  );
