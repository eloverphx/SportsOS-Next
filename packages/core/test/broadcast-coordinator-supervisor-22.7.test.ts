...
describe("Milestone 22.7 coordinator supervisor", () => {
  ...
  const audit = fs.readFileSync(
    new URL("../../../apps/api/src/services/broadcastCoordinatorAudit.ts", import.meta.url),
    "utf8",
  );
  ...
  it("supports bounded actions", () => {
    ["NONE", "RETRY_EXECUTED", "RECONCILED", "REFUSED"].forEach((x) =>
      expect(service).toContain(`"${x}"`),
    );
  });
