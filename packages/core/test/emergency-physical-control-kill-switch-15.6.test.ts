...
describe("Milestone 15.6 emergency physical control kill switch", () => {
  const service = fs.readFileSync(
    new URL("../../../apps/api/src/services/scoreboardEmergencyControlLock.ts", import.meta.url),
    "utf8",
  );
  ...
  it("persists emergency lock state", () => {
    expect(service).toContain("scoreboard-emergency-control-lock.json");
  ...
  it("requires policy permissions to operate emergency lock", () => {
    expect(policyRoute).toContain("/scoreboard-control-emergency-lock");
  ...
  it("requires a reason when activating the emergency lock", () => {
    expect(policyRoute).toContain("A reason is required to activate the emergency lock.");
  });

  it("rejects physical mutation before execution while emergency lock is active", () => {
    const lockIndex = inputRoute.indexOf("emergencyPhysicalControlLock");
  ...
  it("exposes emergency lock controls in operator UI", () => {
    expect(panel).toContain("Emergency Physical Control Lock");
    expect(panel).toContain("Activate Emergency Lock");
    expect(panel).toContain("Clear Emergency Lock");
  });
});
