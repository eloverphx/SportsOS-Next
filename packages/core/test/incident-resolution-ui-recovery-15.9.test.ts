...
describe("Milestone 15.9 incident-resolution UI recovery", () => {
  ...
  it("restores incident-resolution type and state", () => {
    expect(panel).toContain("type IncidentResolution");

    expect(panel).toContain("incidentResolutions");

    expect(panel).toContain("incidentNotes");
  });

  it("loads incident-resolution inventory", () => {
    expect(panel).toContain("/scoreboard-control-incident-resolutions");
  });

  it("restores acknowledge and resolve actions", () => {
    expect(panel).toContain("updateIncidentResolution");
  ...
  it("shows latest incident resolution state", () => {
    expect(panel).toContain("resolutionForIncident");

    expect(panel).toContain("Latest note:");
  });
});
