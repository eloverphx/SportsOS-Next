...
describe("Milestone 13.10 firmware fleet acceptance / closeout", () => {
  it("adds a single fleet acceptance runner", () => {
    const script = fs.readFileSync(
      new URL("../../../firmware/esp32-scoreboard/run-fleet-acceptance.sh", import.meta.url),
      "utf8",
    );
    ...
    expect(script).toContain("node --test firmware/esp32-scoreboard/simulator/test/*.test.js");

    expect(script).toContain("build-in-docker.sh");
  ...
  it("documents release, rollout, OTA, and reporting acceptance", () => {
    const checklist = fs.readFileSync(
      new URL("../../../firmware/esp32-scoreboard/FLEET-ACCEPTANCE-CHECKLIST.md", import.meta.url),
      "utf8",
    );
  ...
  it("requires verified devices throughout fleet management", () => {
    ...
    expect(checklist).toContain("Device must be VERIFIED.");

    expect(checklist).toContain("All targets must be VERIFIED devices.");
  });
  ...
  it("documents the Milestone 13 closeout sequence", () => {
    ...
    expect(readme).toContain("Milestone 13.10");

    expect(readme).toContain("run-fleet-acceptance.sh");
  });
});
