...
describe("Milestone 11.10 firmware diagnostics closeout", () => {
  it("defines a firmware diagnostic snapshot", () => {
    const header = fs.readFileSync(
      new URL("../../../firmware/esp32-scoreboard/include/FirmwareDiagnostics.h", import.meta.url),
      "utf8",
    );
  ...
  it("maps diagnostic connection and health states to text", () => {
    const source = fs.readFileSync(
      new URL("../../../firmware/esp32-scoreboard/src/FirmwareDiagnostics.cpp", import.meta.url),
      "utf8",
    );
  ...
  it("includes a real-hardware validation checklist", () => {
    ...
    expect(checklist).toContain("Power and boot");
    expect(checklist).toContain("Authoritative synchronization");
    expect(checklist).toContain("Fault recovery");
    expect(checklist).toContain("Release gate");
  });
  ...
  it("extends the host simulator with diagnostic behavior", () => {
    ...
    expect(simulator).toContain("buildDiagnosticSnapshot");
    expect(simulator).toContain("authoritativeStateStale");
    expect(simulator).toContain("recoveryRequired");
  });
  ...
  it("documents PlatformIO and physical flashing as remaining hardware gates", () => {
    ...
    expect(readme).toContain("PlatformIO compilation and real ESP32 flashing");
  });
});
