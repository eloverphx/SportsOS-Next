...
describe("Milestone 18.8 preflight countdown / start-window guidance", () => {
  ...
  it("updates the countdown every second", () => {
    expect(panel).toContain("Preflight countdown clock");
  ...
  it("calculates remaining freshness from server expiration", () => {
    expect(panel).toContain("remainingFreshnessMs");

    expect(panel).toContain("freshness.expiresAt");
  });
  ...
  it("warns when expiration is approaching", () => {
    expect(panel).toContain("close to expiration");

    expect(panel).toContain("start window is getting short");
  });

  it("requires rerun after expiration", () => {
    expect(panel).toContain("Rerun it before game start.");
  });
});
