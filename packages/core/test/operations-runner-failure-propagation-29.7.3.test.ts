...
describe("Milestone 29.7.3 operations runner failure propagation", () => {
  ...
  it("returns the child command failure from run_step", () => {
    ...
    expect(runner).toContain('echo "FAIL $label (exit=$rc)"');
  });

  it("does not unconditionally print PASS after a child command", () => {
    const match = runner.match(/run_step\(\)\s*\{([\s\S]*?)^\}/m);
  ...
  it("fails fast from run_step calls", () => {
    const outsideFunction = runner.replace(/run_step\(\)\s*\{[\s\S]*?^\}/m, "");

    const calls = outsideFunction.split("\n").filter((line) => /^\s*run_step\s+/.test(line));
