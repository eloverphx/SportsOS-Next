...
import { evaluateSessionInvalidationReadiness } from "../../../apps/api/src/services/sessionInvalidationReadiness";
...
describe("Milestone 26.9 security attack-surface regression", () => {
  ...
  it("requires production runtime for token invalidation readiness", () => {
    expect(
      evaluateSessionInvalidationReadiness({
        ...
        JWT_SECRET: "abcdefghijklmnopqrstuvwxyz0123456789-strong",
      }).ready,
    ).toBe(false);
  });
  ...
  it("keeps security dashboard read-only", () => {
    const page = fs.readFileSync(
      new URL("../../../apps/dashboard/app/broadcast/security/page.tsx", import.meta.url),
      "utf8",
    );
