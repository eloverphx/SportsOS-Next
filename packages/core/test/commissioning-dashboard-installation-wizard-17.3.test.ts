...
describe("Milestone 17.3 commissioning dashboard / installation wizard", () => {
  ...
  it("starts a commissioning record by device ID", () => {
    expect(panel).toContain("Start Commissioning");

    expect(panel).toContain("/scoreboard-device-commissioning/");
  });
  ...
  it("allows only physical installation confirmations to be manually toggled", () => {
    expect(panel).toContain('step.id ===\n                    "FLASHED"');

    expect(panel).toContain('step.id ===\n                      "PROVISIONED"');

    expect(panel).toContain("Confirm Complete");
  });

  it("runs automated validation from the wizard", () => {
    expect(panel).toContain("/validate");

    expect(panel).toContain("Run Validation");
  });

  it("surfaces final game-ready state", () => {
    expect(panel).toContain("GAME_READY");

    expect(panel).toContain("passed the full commissioning workflow");
  });

  it("renders the wizard on scoreboard operations", () => {
    expect(page).toContain("ScoreboardCommissioningWizard");
