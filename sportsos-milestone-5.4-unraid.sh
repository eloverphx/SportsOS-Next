...
for f in "$PAGE" "$CSS" "$TEST"; do
  if [[ ! -f "$f" ]]; then
    echo "Missing expected 5.3 file: $f" >&2
    exit 1
  fi
done
...
BACKUP_DIR=".game-engine-backups/5.4-${STAMP}"
...
describe("scorekeeper console vertical-slice contract", () => {
  ...
  it("exempts external teams from roster readiness requirements", () => {
    expect(source).toContain("game.homeTeamId === null");
    expect(source).toContain("game.awayTeamId === null");
  ...
  it("loads roster-aware event players and active penalties", () => {
    expect(source).toContain("`/games/${gameId}/event-players`");
    expect(source).toContain("`/games/${gameId}/penalties`");
  });

  it("records roster-aware goals and penalties as game events", () => {
  ...
  it("uses authoritative endpoints for event undo and penalty clearing", () => {
...
echo "  $BACKUP_DIR"
echo
echo "Required START GAME checks:"
echo "  scheduled + PREGAME game state"
echo "  GAME_SCORE permission"
echo "  realtime connection"
echo "  registered home roster has players"
echo "  registered away roster has players"
echo
echo "Recommended / non-blocking checks:"
echo "  scoreboard assigned"
echo "  scoreboard online"
echo "  overlay route available"
echo "  stream telemetry explicitly marked not instrumented"
echo
echo "START GAME uses:"
echo "  POST /games/:id/lifecycle"
echo "  command=startGame"
echo "  unique commandId"
echo
echo "Run:"
